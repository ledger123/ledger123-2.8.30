...
sub form_header {
  ...
  # bp 2014/11 JPY have 0 decimals....
  # Unless you enter lastcost with decimals all vendor prices in foreign currency are rounded with 0 decimals
  # are rounded with 0 decimals. If you add decimals to the sellprice you can also add decimals to the customerprice.
  # There are good reasons for sellprice/lastcost to have more decimals than the currency. But this should not
  # depend on the lastcost/sellprice.
  # $form->{maxpricedecimals} is retrieved by the get_defaults function in Form.pm

  for (qw(listprice sellprice)) { $form->{$_} = $form->format_amount(\%myconfig, $form->{$_}, $form->{maxpricedecimals}) }

  for (qw(lastcost avgcost)) { $form->{$_} = $form->format_amount(\%myconfig, $form->{$_}, $form->{maxpricedecimals}) }

  for (qw(customerprice)) { $form->{$_customer} = $form->format_amount(\%myconfig, $form->{$_customer}, $form->{maxpricedecimals}, "SZ" ) }
  for (qw(lastcost)) { $form->{$_vendor} = $form->format_amount(\%myconfig, $form->{$_vendor}, $form->{maxpricedecimals}, "SZ" ) }
...
|;
  ...
  $lastcost = qq|
		...
		<td><input name=markup class="inputright" size=5 value=$form->{markup}>
...
|;
  ...
  $form->hide_form(qw(id item title makemodel alternate onhand orphaned taxaccounts rowcount project_id precision maxpricedecimals changeup oldpartsgroup oldpartsgroupcode helpref));
...
|;
  ...
  $form->hide_form(map { "select$_" } qw(currency partsgroup assemblypartsgroup vendor customer pricegroup maxpricedecimals IC_inventory IC_income IC_expense));
...
sub generate_report {
  ...
  # get maxpricedecimals
  my $dbh = $form->dbconnect(\%myconfig);
  %res = $form->get_defaults($dbh, \@{['maxpricedecimals']});
  $form->{maxpricedecimals} = $res{maxpricedecimals};
  ...
  $hdr{shipped} = { label => $locale->text('Ship/Rec'), align => r, type => n };
  $column_data{shipped} = qq|<th class=listheading nowrap>$hdr{shipped}{label}</th>|;
  ...
  $hdr{cost} = { label => $locale->text('Cost'), align => r, type => n, precision => $form->{maxpricedecimals} };
  $column_data{cost} = qq|<th class=listheading nowrap>$hdr{cost}{label}</th>|;
  $hdr{listprice} = { label => $locale->text('List Price'), align => r, type => n, precision => $form->{maxpricedecimals} };
  $column_data{listprice} = qq|<th class=listheading nowrap>$hdr{listprice}{label}</th>|;
  $hdr{lastcost} = { label => $locale->text('Last Cost'), align => r, type => n, precision => $form->{maxpricedecimals} };
  $column_data{lastcost} = qq|<th class=listheading nowrap>$hdr{lastcost}{label}</th>|;
  $hdr{avgcost} = { label => $locale->text('Avg Cost'), align => r, type => n, precision => $form->{maxpricedecimals} };
  ...
  $hdr{sellprice} = { label => $locale->text('Sell Price'), align => r, type => n, precision => $form->{maxpricedecimals} };
  ...
  for (qw(sellprice lastcost avgcost listprice cost)) {
    # bp2014/11 linetotals are rounded with the doc precision
...
|;
  ...
  foreach $ref (@{ $form->{parts} }) {
	...
	$ref->{sellprice} = $summary{$ref->{id}}{total} / $ref->{onhand};
        $ref->{shipped} = $summary{$ref->{id}}{shipped} / $ref->{onhand};;
    ...
    if (!$form->{summary}) {
      # bp 2014/11 use maxpricedecimals
      for (qw(listprice cost sellprice lastcost avgcost)) { $ref->{$_} = $form->round_amount($ref->{$_}, $form->{maxpricedecimals}) }
    }
    ...
    if ($form->{summary} && $form->{l_linetotal}) {
      $column_data{sellprice} = "<td align=right>".$form->format_amount(\%myconfig, $ref->{sellprice}, $form->{maxpricedecimals}, "&nbsp;") . "</td>";
    } else {
      $column_data{sellprice} = "<td align=right>".$form->format_amount(\%myconfig, $ref->{sellprice}, $form->{maxpricedecimals}, "&nbsp;") . "</td>";
    }
    for (qw(listprice lastcost avgcost cost)) { $column_data{$_} = "<td align=right>".$form->format_amount(\%myconfig, $ref->{$_}, $form->{maxpricedecimals}, "&nbsp;") . "</td>" }

    for (qw(lastcost avgcost)) { $column_data{"${_}markup"} = "<td align=right>".$form->format_amount(\%myconfig, $ref->{"${_}markup"}, 1, "&nbsp;")."</td>" }

    if ($form->{l_linetotal}) {
      for (qw(listprice cost)) { $column_data{"linetotal$_"} = "<td align=right>".$form->format_amount(\%myconfig, $ref->{onhand} * $ref->{$_}, $form->{precision}, "&nbsp;")."</td>" }
      for (qw(sellprice lastcost avgcost)) { $column_data{"linetotal$_"} = "<td align=right>".$form->format_amount(\%myconfig, $ref->{onhand} * $ref->{$_}, $form->{maxpricedecimals}, "&nbsp;")."</td>" }
...
|;
  ...
  if ($form->{"l_linetotal"}) {
    ...
    # bp 2014/11 round line totals with the doc precision
...
carp("mpricedec: $form->{maxpricedecimals} \n");
...
#  $form->{maxpricedecimals} = $res{maxpricedecimals};
  ...
  foreach $ref (@{ $form->{parts} }) {
    ...
    for (qw(sell sellprice listprice lastcost avgcost)) { $ref->{$_} = $form->format_amount(\%myconfig, $ref->{$_}, $form->{maxpricedecimals}) }
...
carp("maxpricedec: $form->{maxpricedecimals} \n");
...
|;
	...
	<tr>
	  $vendor
	  <td>$fld{partnumber}</td>
	  <td><input name="lastcost_$i" class="inputright" size=11 value=|.$form->format_amount(\%myconfig, $form->{"lastcost_$i"}, $form->{maxpricedecimals}, "SZ").qq|></td>
	  $currency
	  <td nowrap><input name="leadtime_$i" class="inputright" size=5 value=|.$form->format_amount(\%myconfig, $form->{"leadtime_$i"}, 0).qq|> <b>|.$locale->text('days').qq|</b></td>
	</tr>
|;
...
|;
	...
	<tr>
	  ...
	  <td><input name="customerprice_$i" class="inputright" size=11 value=|.$form->format_amount(\%myconfig, $form->{"customerprice_$i"}, $form->{maxpricedecimals}, "SZ" ).qq|></td>
	  $currency
...
|;
  ...
  for $i (1 .. $numrows) {
    ...
    # bp 2014/11 line totals are rounded with the doc precision
    $linetotalsellprice = $form->round_amount($form->{"sellprice_$i"} * $form->{"qty_$i"}, $form->{precision});
    $form->{sellprice} += $linetotalsellprice;

    $linetotallistprice = $form->round_amount($form->{"listprice_$i"} * $form->{"qty_$i"}, $form->{precision});
    $form->{listprice} += $linetotallistprice;

    $linetotallastcost = $form->round_amount($form->{"lastcost_$i"} * $form->{"qty_$i"}, $form->{precision});
    ...
    if ($i == $numrows) {
      ...
      # bp 2014/11 line totals are rounded with the doc precision
      $linetotalsellprice = $form->format_amount(\%myconfig, $linetotalsellprice, $form->{precision}, 0);
      $linetotallistprice = $form->format_amount(\%myconfig, $linetotallistprice, $form->{precision}, 0);
      $linetotallastcost = $form->format_amount(\%myconfig, $linetotallastcost, $form->{precision}, 0);
...
|;
  ...
  $column_data{sellprice} = "<th align=right>".$form->format_amount(\%myconfig, $form->{sellprice}, $form->{maxpricedecimals})."</th>";
  $column_data{listprice} = "<th align=right>".$form->format_amount(\%myconfig, $form->{listprice}, $form->{maxpricedecimals})."</th>";
  $column_data{lastcost} = "<th align=right>".$form->format_amount(\%myconfig, $form->{lastcost}, $form->{maxpricedecimals})."</th>";
...
#carp("ic.pl; function:save - partsgroup: $form->{partsgroup} - id: $form->{id} - partsgroup_id: $form->{partsgroup_id} \n");
  ...
  if ($form->{previousform} && !$form->{callback}) {
    ...
    if ($form->{item} eq 'assembly') {
      ...
      foreach $item (qw(sellprice lastcost)) {
        $form->{$item} += $form->{"${item}_$i"} * $form->{"qty_$i"};
        $form->{$item} = $form->round_amount($form->{$item}, $form->{maxpricedecimals});
      }

# bp 2014/11
      $form->{listprice} += $form->{"listprice_$i"} * $form->{"qty_$i"};
      $form->{listprice} = $form->round_amount($form->{listprice}, $form->{maxpricedecimals});
	...
	$form->{"sellprice_$i"} = $form->round_amount($form->{"sellprice_$i"} / $form->{exchangerate}, $form->{maxpricedecimals});
      }
