...
sub edit {
  ...
  for ( @{ $form->{PR}{"$form->{ARAP}_discount"} } ) {
    $form->{"select$form->{ARAP}_discount"} .= "$_->{accno}--$_->{description}\n";
  }
  for ( @{ $form->{PR}{"$form->{ARAP}_paid"} } ) {
    $form->{"select$form->{ARAP}_paid"} .= "$_->{accno}--$_->{description}\n";
  }
  ...
  for ( "$form->{vc}", "$form->{ARAP}", "$form->{ARAP}_paid", "$form->{ARAP}_discount" ) {
    $form->{"select$_"} = $form->escape( $form->{"select$_"}, 1 );
  }
  for (qw(currency department business language account)) {
    $form->{"select$_"} = $form->escape( $form->{"select$_"}, 1 );
  }
  ...
  if ( @{ $form->{transactions} } ) {
    ...
    foreach $ref ( @{ $form->{transactions} } ) {
      ...
      for (qw(id invnumber invdescription transdate duedate calcdiscount discountterms cashdiscount)) {
        $form->{"${_}_$i"} = $ref->{$_};
      }
      $ref->{exchangerate} ||= 1;
      ...
      for (qw(amount paid due discount total)) {
        $form->{"${_}_$i"} = $form->format_amount( \%myconfig, $ref->{$_}, $form->{precision} );
      }
      $form->{"checked_$i"} = 1;
    }
  }
...
sub payment {
  ...
  $form->{callback} =
    "$form->{script}?action=payment&path=$form->{path}&login=$form->{login}&all_vc=$form->{all_vc}&type=$form->{type}"
  ...
  for (qw(currency department business language account paymentmethod printer)) {
    $form->{"select$_"} = $form->escape( $form->{"select$_"}, 1 );
  }
...
sub prepare_payments_header {
  ...
  for ( "currency",   "$form->{ARAP}", "$form->{ARAP}_paid", "$form->{ARAP}_discount",
        "department", "business",      "paymentmethod" )
  {
    $form->{"old$_"} = $form->{$_};
  }
  ...
  foreach $ref ( @{ $form->{PR} } ) {
    ...
    for (qw(id invnumber invdescription transdate duedate calcdiscount discountterms cashdiscount netamount)) {
      $form->{"${_}_$i"} = $ref->{$_};
    }
    $ref->{exchangerate} ||= 1;
    ...
    $form->{"amount_$i"} =
      $form->format_amount( \%myconfig, $ref->{amount} / $ref->{exchangerate}, $form->{precision} );
...
sub payments {
  ...
  $form->{callback} = "$form->{script}?action=payments&path=$form->{path}&login=$form->{login}&type=$form->{type}"
    unless $form->{callback};
...
|;
  ...
  $form->hide_form(
    qw(defaultcurrency closedto vc type formname arap ARAP title payment batch batchid batchnumber batchdescription transdate edit voucherid deselect employee cdt precision)
  );
...
|;
  ...
  $column_data{checked} =
qq|<th><input name="allbox_select" type=checkbox class=checkbox value="1" $form->{allbox_select} onChange="CheckAll(); javascript:document.forms[0].submit()" ><input type=hidden name=action value="$action"></th>|;
...
|;
  ...
  for $i ( 1 .. $form->{rowcount} ) {
    ...
    for (qw(amount due paid)) {
      $form->{"${_}_$i"} = $form->format_amount( \%myconfig, $form->{"${_}_$i"}, $form->{precision} );
    }
    ...
    $column_data{detail} =
      qq|<td align=center><input name="detail_$i" type=checkbox class=checkbox $form->{"detail_$i"}></td>|;

    $column_data{"$form->{vc}number"} = qq|<td>$form->{"$form->{vc}number_$i"}</td>|;
    $column_data{name} = qq|<td>$form->{"name_$i"}</td>|;

    $column_data{memo} =
      qq|<td align=center><input name="memo_$i" size=20 value="| . $form->quote( $form->{"memo_$i"} ) . qq|"></td>|;
    $column_data{source} =
      qq|<td align=center><input name="source_$i" size=10 value="| . $form->quote( $form->{"source_$i"} ) . qq|"></td>|;

    if ( $form->{selectlanguage} ) {
      $column_data{language} =
          qq|<td><select name="language_code_$i">|
...
|;
  ...
  $column_data{amount} =
    ...
    . $form->format_amount( \%myconfig, $totalamount, $form->{precision}, "&nbsp;" )
    . qq|</th>|;
  $column_data{due} =
      qq|<th class=listtotal align=right>|
    . $form->format_amount( \%myconfig, $totaldue, $form->{precision}, "&nbsp;" )
    . qq|</th>|;
  $column_data{paid} =
      qq|<th class=listtotal align=right>|
...
sub payments_footer {
 ...
 <option value=screen>| . $locale->text('Screen');
  if ( $form->{selectprinter} ) {
    for ( split /\n/, $form->unescape( $form->{selectprinter} ) ) {
      $media .= qq|
 <option value="$_">$_|;
    }
  }
...
sub update_payment {
  ...
  # bp 2014/12 ensure that we have the right precision
  if ( $form->{currency} ne $form->{defaultcurrency} ) {
  ...
  if ( !$form->{all_vc} ) {
         ...
         || $form->{business} ne $form->{oldbusiness} )
    {
     ...
     # bp 2014/12 retrieve open invoices only if we have a customer number
     $form->{"select$form->{vc}"} = $form->unescape($form->{"select$form->{vc}"} );
     ( $d, $id ) = split /--/, $form->{"select$form->{vc}"} ;
    if ( $id > 0 ) {
  ...
  if ( $new_name_selected || $form->{redo} ) {
    ...
    # bp 2014/12 new customer selected, document for the selected currency are shown -> set the matching precision
    if ($form->{currency} ne $form->{defaultcurrency} ) {
      $form->{precision} = $form->get_precision( \%myconfig, $form->{currency} );
    }
    ...
    # bp 2014/12 keep a manually entered exchangerate
    if ( $form->{datepaid} ne $form->{olddatepaid} || $form->{currency} ne $form->{oldcurrency} ||
         ( $form->{exchangerate} eq '' && $form->{currency} ne $form->{defaultcurrency} ) ) {
      $form->{exchangerate} = $form->check_exchangerate( \%myconfig, $form->{currency}, $form->{datepaid} );
    }
  ...
  if ( $form->{redo} ) {
    ...
    foreach $ref ( @{ $form->{PR} } ) {
      ...
      $form->{"amount_$i"} =
        $form->format_amount( \%myconfig, $ref->{amount} / $ref->{exchangerate}, $form->{precision} );
      $form->{"netamount_$i"} =
        $form->format_amount( \%myconfig, $ref->{netamount} / $ref->{exchangerate}, $form->{precision} );
    ...
    if ( $form->{"checked_$i"} ) {
      ...
      if ( $form->{"calcdiscount_$i"} ) {
        if ( !$form->{"olddiscount_$i"} ) {
          $form->{"discount_$i"} =
#            $form->parse_amount( \%myconfig, $form->{"netamount_$i"} ) * $form->{"cashdiscount_$i"};
            $form->parse_amount( \%myconfig, $form->{"amount_$i"} ) * $form->{"cashdiscount_$i"};
          $form->{"olddiscount_$i"} = $form->{"discount_$i"};
        }
      }
    ...
    for (qw(amount due paid discount)) {
      $form->{"${_}_$i"} = $form->format_amount( \%myconfig, $form->{"${_}_$i"}, $form->{precision} );
    }
  }
...
#say Dumper ( $form->{business_type} , $form );

  # bp 2014/11 select business type and payment method fields if not set
  $form->{department}      = $form->{selectdepartment}  if ( ! $form->{department} && $form->{department_id} );
#  $form->{business}      = $form->{business_type}  if ( ! $form->{business} && $form->{business_id} );
  $form->{paymentmethod} = $form->{payment_method} if ( ! $form->{paymentmethod} && $form->{paymentmethod_id} );
  $form->{business}      = $form->{business_type}  if ( ! $form->{business} && $form->{business_id} );
...
|;
 ...
 |;
      ...
      $vc .=
          qq|<td><input id="vc" name="$form->{vc}" size=35 value="|
        . $form->quote( $form->{ $form->{vc} } )
        . qq|"></td>
 </tr>
 <tr>
 <th align=right>| . $locale->text( $vc{ $form->{vc} }{number} ) . qq|</th>
 <td><input id="vcnumber" name="$form->{vc}number" size=35 value="|
        . $form->quote( $form->{"$form->{vc}number"} )
        . qq|"></td>
 </tr>
 |;
    }
...
|;
  ...
  $form->hide_form(
    qw(defaultcurrency closedto vc type ARAP arap title formname payment batch batchid batchnumber batchdescription transdate edit voucherid vouchernumber deselect employee precision)
  );
  ...
  $form->hide_form( map { "select$_" }
                    ( "$form->{ARAP}", "$form->{ARAP}_paid", "$form->{ARAP}_discount", "$form->{vc}" ) );
...
<table width=100%>
 ...
 <td colspan=3><input name=amount class="inputright" size=11 value=|
...
|;
  ...
  # bp 2014/11 added business_id and paymentmethod_id
  $form->hide_form(qw(address1 address2 city state zipcode country business_id paymentmethod_id));
...
|;
  ...
  for $i ( 1 .. $form->{rowcount} ) {
    ...
    for (qw(amount due paid discount total)) {
      $form->{"${_}_$i"} = $form->format_amount( \%myconfig, $form->{"${_}_$i"}, $form->{precision} );
    }
    ...
    $form->hide_form( map { "${_}_$i" }
      qw(id invnumber invdescription transdate duedate due calcdiscount discountterms cashdiscount amount netamount olddiscount)
    );
    ...
    if ( $form->{"calcdiscount_$i"} ) {
      $column_data{discount} =
        qq|<td align=center><input name="discount_$i" class="inputright" size=11 value=$form->{"discount_$i"}></td>|;
    } else {
...
|;
  ...
  $column_data{discount} =
      qq|<th class=listtotal align=right>|
    . $form->format_amount( \%myconfig, $totaldiscount, $form->{precision}, "&nbsp;" )
    . qq|</th>|;
  $column_data{amount} =
      qq|<th class=listtotal align=right>|
    . $form->format_amount( \%myconfig, $totalamount, $form->{precision}, "&nbsp;" )
  ...
  $column_data{total} =
      qq|<th class=listtotal align=right>|
    . $form->format_amount( \%myconfig, $totaltotal, $form->{precision}, "&nbsp;" )
    . qq|</th>|;
...
sub post_payments {
  ...
  foreach $ref ( @{ $form->{PR} } ) {
    ...
    $form->{"paid_$i"} =
      $form->format_amount( \%myconfig, $form->{"paid_$i"} - $form->{"discount_$i"}, $form->{precision} );
...
sub post_payment {
  ...
  # bp 2014/12 if "post" is pressed without pressing "update" beforehand the paymentamount
  # will be zero and an overpayment document with a negative amount will be created
  $form->error( $locale->text('Payment amount missing! Press Update before Post!') ) unless $form->{amount};
