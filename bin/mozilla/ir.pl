...
#	$form->{"paid_$i"} = $form->{acc_trans}{$key}->[$i-1]->{amount} * $ml;
  # bp 2014/11 see create_links in Form.pm
...
sub update {
  ...
  $form->{creditremaining} -= ($amount * $ml);
	...
	for (qw(sell sellprice listprice, lastcost)) { $form->{"${_}_$i"} = $form->format_amount(\%myconfig, $form->{"${_}_$i"}, $form->{maxpricedecimals}) }
...
sub post {
 ...
 # bp 2014/11 $roundto is defined in document or by payment method. Can be set to values between 0.01 and 1.0
 #            cashover arises if $roundto e.g. 0.1 is higher than the currency precision e.g. 2 (0.01)
 #            If the home currency is Jpy this is not possible (neither invoicetotal nor payment less then 1 Jpy)
  if ($roundto > 0.01) {
