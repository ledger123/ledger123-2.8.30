...
# bp2014/11 if the condition is enabled the transport part box will only show if
# transport companies have been set up.
...
sub include_in_report {
  ...
  if ($form->{db} eq 'customer') {
    ...
    push @f, qq|<input name="l_transportcompany" type=checkbox class=checkbox value=Y> |.$locale->text('Transport Company');
  }
...
sub list_names {
  ...
  push @columns, qw(bankcity, bankstate, bankzipcode, bankcountry, iban, bic, remittancevoucher,
              membernumber, clearingnumber, transportcompany, transport, startdate, enddate,
              invnumber, invamount, invtax, invtotal, ordnumber, ordamount, ordtax, ordtotal,
...
#carp("ct.pl form_header: now selecting transport partner \n");
