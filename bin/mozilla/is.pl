...
sub invoice_links {
  ...
  foreach $key (keys %{ $form->{AR_links} }) {
	...
	# reverse paid
      ...
      # TODO if there are no fxamounts we have to retrieve the acc_trans lines with FX_trans = true!!
      # bp 2014/12 amounts are now in home currency - we always convert, exchange rate for home currency is 1 !
      $form->{"paid_$i"} = convert ( $form->{"paid_$i"}, $form->{"exchangerate_$i"} );
      ...
      $form->{"discount_paymentmethod"} = "$form->{acc_trans}{$key}->[0]->{paymentmethod_id}--$form->{acc_trans}{$key}->[0]->{paymentmethod}";
        ...
        # TODO if there are no fxamounts we have to retrieve the acc_trans lines with FX_trans = true!!
        # bp 2014/12 amounts are now in home currency - we always convert, exchange rate for home currency is 1 !
        $form->{"discount_paid"} = convert ( $form->{"discount_paid"}, $form->{"discount_exchangerate"} );
...
.qq|\nbarcode--|.$locale->text('Barcode');
  ...
  if ($form->{id}) {
    ...
    foreach $ref (@{ $form->{invoice_details} } ) {
      ...
      $form->{"sellprice_$i"} = $form->format_amount(\%myconfig, $form->{"sellprice_$i"}, $form->{maxpricedecimals} );
  ...
  if ($form->{defaultcurrency}) {
    ...
    if ($form->{currency} ne $form->{defaultcurrency}) {
      ...
      $exchangerate .= qq|
	      ...
	      <td nowrap><input name="exchangerate" id="exrate" class="inputright" size="10" value="$form->{exchangerate}">
...
|;
  ...
  $form->hide_form(qw(id type printed emailed queued title vc discount creditlimit creditremaining tradediscount business closedto locked shipped oldtransdate oldduedate recurring defaultcurrency oldterms cdt precision maxpricedecimals order_id remittancevoucher reference_rows referenceurl));
