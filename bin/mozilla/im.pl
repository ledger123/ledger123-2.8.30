...
|;
    ...
    $i = 1;
    $includeinreport{accountnumber} =
      { ndx => $i++, input => 1, html => qq|<input name="l_accountnumber" class=checkbox type=checkbox value=Y $form->{l_accountnumber}>|, label => $locale->text('Account Number') };
    ...
    $includeinreport{paymentmethod} =
      { ndx => $i++, input => 1, html => qq|<input name="l_paymentmethod" class=checkbox type=checkbox value=Y $form->{l_paymentmethod}>|, label => $locale->text('Payment Method') };
    $includeinreport{dateprepared} =
      { ndx => $i++, input => 1, html => qq|<input name="l_dateprepared" class=checkbox type=checkbox value=Y $form->{l_dateprepared}>|, label => $locale->text('Date Prepared') };

    $includeinreport{invnumber} =
      { ndx => $i++, checkbox => 1, html => qq|<input name="l_invnumber" class=checkbox type=checkbox value=Y $form->{l_invnumber}>|, label => $locale->text('Invoice Number') };
    $includeinreport{description} =
      { ndx => $i++, checkbox => 1, html => qq|<input name="l_description" class=checkbox type=checkbox value=Y $form->{l_description}>|, label => $locale->text('Description') };
    ...
    $includeinreport{companynumber} =
      { ndx => $i++, checkbox => 1, html => qq|<input name="l_companynumber" class=checkbox type=checkbox value=Y $form->{l_companynumber}>|, label => $locale->text('Customer/Vendor Number') };
    $includeinreport{address1} =
      { ndx => $i++, checkbox => 1, html => qq|<input name="l_address1" class=checkbox type=checkbox value=Y $form->{l_address1}>|, label => $locale->text('Address Line 1') };
    $includeinreport{address2} =
      { ndx => $i++, checkbox => 1, html => qq|<input name="l_address2" class=checkbox type=checkbox value=Y $form->{l_address2}>|, label => $locale->text('Address Line 2') };
    ...
    $includeinreport{clearingnumber} =
      { ndx => $i++, checkbox => 1, html => qq|<input name="l_clearingnumber" class=checkbox type=checkbox value=Y $form->{l_clearingnumber}>|, label => $locale->text('BC Number') };
    $includeinreport{membernumber} =
      { ndx => $i++, checkbox => 1, html => qq|<input name="l_membernumber" class=checkbox type=checkbox value=Y $form->{l_membernumber}>|, label => $locale->text('Member Number') };
    ...
    $includeinreport{company} =
      { ndx => $i++, checkbox => 1, html => qq|<input name="l_company" class=checkbox type=checkbox value=Y $form->{l_company}>|, label => $locale->text('Our Company Name') };
...
|;
	...
	<tr>
	  <th align="right" nowrap>| . $locale->text('Currency') . qq|</th>
	  <td><select name=curr>|
              . $form->select_option( $form->{selectcurrency} ) . qq|</select>
	  </td>
...
|;
	...
	<tr>
	  ...
	  <td><select name=paymentmethod>|
                  . $form->select_option( $form->{selectpaymentmethod}, undef, 1 ) . qq|</select></td>
...
|;
    ...
    for ( split /\n/, $form->{address} ) {
        $j++;
        $includeinreport{"companyaddress$j"} = {
            ndx      => $i++,
            checkbox => 1,
            html     => qq|<input name="l_companyaddress$j" class=checkbox type=checkbox value=Y $form->{"l_companyaddress$j"}>|,
            label    => $locale->text('Our Address Line') . " $j"
        };
    }

    $includeinreport{accountclearingnumber} =
      { ndx => $i++, checkbox => 1, html => qq|<input name="l_accountclearingnumber" class=checkbox type=checkbox value=Y $form->{l_accountclearingnumber}>|, label => $locale->text('Our BC Number') };
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
      ...
      . $form->select_option( $selectdateformat, $form->{dateformat} ) . qq|
	  </td>
	</tr>
      </table>
    </td>
    ...
    for my $i ( 1 .. $form->{rowcount} ) {
        ...
        if ( $form->{"ndx_$i"} ) {
            ...
            if ( CP->post_payment( \%myconfig, \%$newform ) ) {
                $form->info(
                    qq| $form->{"invnumber_$i"}, $form->{"description_$i"}, $form->{"companynumber_$i"}, $form->{"name_$i"}, $form->{"city_$i"}, $form->{"amount_$i"} ... | . $locale->text('ok') );
...
|;
    ...
    %button = (
        'part'    => { ndx => 1, key => 'I', value => $locale->text('Import Parts') },
...
|;
            ...
            for ( 1 .. $#column_index ) {
                ...
                $column_data{ $column_index[$_] } =
                    qq|<th align="$form->{"h_a_$_"}"><a class=listtop href="$href&editcolumn=$column_index[$_],$_,h">|
                  . $form->pad( $f, $form->{"h_f_$_"}, $form->{"h_a_$_"}, $form->{"h_w_$_"}, 1 )
                  . qq|</a></th>|;

            }

            $column_data{dateprepared} =
              qq|<th nowrap align="$form->{"h_a_$column_index{dateprepared}"}"><a class=listtop href="$href&editcolumn=dateprepared,$column_index{dateprepared},h">$hdateprepared</th>|
              if $form->{"h_w_$column_index{dateprepared}"};
...
|;
        ...
        for (@column_index) {
            ...
            $column_data{$_} =
              qq|<td nowrap align="$form->{"a_$column_index{$_}"}">| . $form->pad( $f, $form->{"f_$column_index{$_}"}, $form->{"a_$column_index{$_}"}, $form->{"w_$column_index{$_}"}, 1 ) . qq|</td>|;
        }
        ...
        if ( $form->{filetype} eq 'txt' ) {
            $column_data{amount} = qq|<td nowrap align="$form->{"a_$column_index{amount}"}">|
              . $form->pad(
                $form->format_amount( \%myconfig, $ref->{amount}, $form->{precision} ),
                $form->{"f_$column_index{amount}"},
                $form->{"a_$column_index{amount}"},
                $form->{"w_$column_index{amount}"}, 1
              ) . qq|</td>|;
        }
        else {
            $column_data{amount} = qq|<td nowrap align="right">|
              . $form->pad(
                $form->format_amount( \%myconfig, $ref->{amount}, $form->{precision} ),
                $form->{"f_$column_index{amount}"},
                $form->{"a_$column_index{amount}"},
...
|;
        ...
        if ( ( $nextday ne $sameday ) && $txtheader ) {
            ...
            for ( 1 .. $#column_index ) {
                ...
                $column_data{ $column_index[$_] } =
                    qq|<th align="$form->{"t_a_$_"}"><a class=listsubtotal href="$href&editcolumn=$column_index[$_],$_,t">|
                  . $form->pad( $f, $form->{"t_f_$_"}, $form->{"t_a_$_"}, $form->{"t_w_$_"}, 1 )
                  . qq|</a></th>|;
            }

            $column_data{dateprepared} =
              qq|<th nowrap align="$form->{"t_a_$column_index{dateprepared}"}"><a class=listsubtotal href="$href&editcolumn=dateprepared,$column_index{dateprepared},t">$tdateprepared</th>|
              if $form->{"t_w_$column_index{dateprepared}"};

            $column_data{amount} = qq|<th align="$form->{"t_a_$column_index{amount}"}"><a class=listsubtotal href="$href&editcolumn=amount,$column_index{amount},t">|
              . $form->pad(
                $form->format_amount( \%myconfig, $subtotal, $form->{precision} ),
                $form->{"t_f_$column_index{amount}"},
                $form->{"t_a_$column_index{amount}"},
                $form->{"t_w_$column_index{amount}"}, 1
              ) . qq|</a></th>|;
    ...
    if ( $form->{filetype} eq 'txt' ) {
        if ( !$txtheader ) {
            ...
            for ( 1 .. $#column_index ) {
                ...
                $column_data{ $column_index[$_] } =
                    qq|<th align="$form->{"t_a_$_"}"><a class=listtotal href="$href&editcolumn=$column_index[$_],$_,t">|
                  . $form->pad( $f, $form->{"t_f_$_"}, $form->{"t_a_$_"}, $form->{"t_w_$_"}, 1 )
                  . qq|</a></th>|;
            }
            ...
            $column_data{amount} = qq|<th align="$form->{"t_a_$column_index{amount}"}"><a class=listtotal href="$href&editcolumn=amount,$column_index{amount},t">|
              . $form->pad(
...
|;
    ...
    $form->hide_form(
        qw(login path report reportcode paymentaccount curr paymentmethod dateprepared dateformat filetype delimiter decimalpoint tabdelimited includeheader stringsquoted defaultcurrency type flds column_index rowcount linefeed callback nextsub title)
    );

    %button = (
        'Export Payments'    => { ndx => 1, key => 'E', value => $locale->text('Export Payments') },
    ...
    foreach $ref ( @{ $form->{TR} } ) {
        if ( $id{ $ref->{id} } ) {
            ...
            if ( ( $nextday ne $sameday ) && $txtheader ) {
                ...
                $form->{"amount_$i"} = $form->pad(
                    $form->format_amount( \%myconfig, $subtotal, $form->{precision} ), $form->{"t_f_$column_index{amount}"},
                    $form->{"t_a_$column_index{amount}"}, $form->{"t_w_$column_index{amount}"}
                ) if $form->{"t_w_$column_index{amount}"};
    ...
    if ( $form->{filetype} eq 'txt' ) {
        ...
        if ( !$txtheader ) {
            ...
            if ($summaryrecord) {
                ...
                $form->{"amount_$i"} = $form->pad(
                    $form->format_amount( \%myconfig, $total, $form->{precision} ), $form->{"t_f_$column_index{amount}"},
                    $form->{"t_a_$column_index{amount}"}, $form->{"t_w_$column_index{amount}"}
                ) if $form->{"t_w_$column_index{amount}"};
