...
sub order_links {
  ...
  # bp 2014/11 enable_selectprinter is set in sql-ledger-sys.conf
...
sub prepare_order {
  ...
  if ( $form->{id} ) {
    ...
    for (
      qw(ordnumber quonumber shippingpoint shipvia waybill notes intnotes
      ...
      transportcontact transportphone transportfax transportemail transportcc transportbcc)
      )
    {
    ...
    foreach $ref ( @{ $form->{form_details} } ) {
      ...
      for ( map { "${_}_$i" } qw(sellprice listprice) ) {
        $form->{$_} = $form->format_amount( \%myconfig, $form->{$_}, $form->{maxpricedecimals} );
      }

      $form->{"lastcost_$i"} = $form->format_amount( \%myconfig, $form->{"lastcost_$i"}, $form->{maxpricedecimals} );
  ...
  if ( $form->{type} eq 'sales_order' ) {
    ...
    $form->{selectformname} =
      ...
      . qq|\ntransport_order--|
      . $locale->text('Transport Order')
      . qq|\ndelivery_list--|
      . $locale->text('Delivery List')
      . qq|\npick_list--|
  ...
  if ( $form->{type} eq 'ship_order' ) {
    $form->{selectformname} = qq|pick_list--| . $locale->text('Pick List') . qq|\npacking_list--| . $locale->text('Packing List') . qq|\nbarcode--| . $locale->text('Barcode');
  }

  if ( $form->{type} eq 'receive_order' ) {
    $form->{selectformname} = qq|bin_list--| . $locale->text('Bin List') . qq|\nbarcode--| . $locale->text('Barcode');
  }
...
sub form_header {
  ...
  # bp 2014/11 add precision data to the form
  $form->{defaultprecision} = $form->get_precision( \%myconfig, $form->{defaultcurrency} );
...
|;
 ...
 <td nowrap><input id="exrate" name=exchangerate class="inputright" size=10 value=$form->{exchangerate}>|;
...
|;
 ...
 <td nowrap>$form->{business}|;
    $business .= qq|&nbsp;&nbsp;&nbsp;
 <b>| . $locale->text('Trade Discount') . qq|</b> | . $form->format_amount( \%myconfig, $form->{tradediscount} * 100 ) . qq| %| if $form->{vc} eq 'customer';
    $business .= qq|</td>
 </tr>
|;
  }
 ...
 <td><input name=ordnumber size=20 value="| . $form->quote( $form->{ordnumber} ) . qq|"></td>| . $form->hide_form(qw(quonumber)) . qq|
 </tr>
 <tr>
...
|;
 ...
 <table>
 <tr>
 <td>| . $form->format_amount( \%myconfig, $form->{creditlimit}, $form->{defaultprecision}, "0" ) . qq|</td>
 <td width=10></td>
 <th align=right nowrap>| . $locale->text('Remaining') . qq|</th>
 <td class="plus$n">| . $form->format_amount( \%myconfig, $form->{creditremaining}, $form->{defaultprecision}, "0" ) . qq|</td>
 </tr>
 </table>
 </td>
 </tr>
|;
 ...
 <td><input name=quonumber size=20 value="| . $form->quote( $form->{quonumber} ) . qq|"></td>| . $form->hide_form(qw(ordnumber)) . qq|
 </tr>
|;
    }
    else {
 ...
 |
        . $form->hide_form(qw(ordnumber)) . qq|
 </tr>
|;
...
| if $form->{selectwarehouse};
 ...
 <td><select name=employee>|
        . $form->select_option( $form->{selectemployee}, $form->{employee}, 1 ) . qq|</select>
 </td>
 </tr>
|;
    }
...
|;
  ...
  $form->hide_form(
    qw(id type defaultcurrency formname printed emailed queued vc title discount creditlimit creditremaining tradediscount business oldtransdate recurring address1 address2 city state zipcode country pricegroup closedto precision maxpricedecimals reference_rows referenceurl)
  );
...
|;
  ...
  $form->hide_form(
    qw(shiptoname shiptoaddress1 shiptoaddress2 shiptocity shiptostate shiptozipcode shiptocountry shiptocontact shiptophone shiptofax shiptoemail message email subject cc bcc taxaccounts aa_id transportname transportaddress1 transportaddress2 transportcity transportstate transportzipcode transportcountry transportcontact transportphone transportfax transportemail transportcc transportbcc)
  );

  foreach $accno ( split / /, $form->{taxaccounts} ) {
    $form->hide_form( map { "${accno}_$_" } qw(rate description taxnumber) );
  }
...
|;
  ...
  if ( $form->{readonly} ) {
    ...
    %button = (
      ...
      'Ship to'               => { ndx => 5,  key => 'T', value => $locale->text('Ship to') },
      'Ship all'              => { ndx => 6,  key => 'A', value => $locale->text('Ship all') },
      'Transport'             => { ndx => 7,  key => 'C', value => $locale->text('Transport') },
    ...
    $f{'Transport'} = 1 if $form->{type} =~ /(sales|purchase)_order/;
...
sub update {
  ...
  $form->{exchangerate} = $form->parse_amount( \%myconfig, $form->{exchangerate}, $form->{maxpricedecimals} );
  ...
  if ( ( $form->{"partnumber_$i"} eq "" ) && ( $form->{"description_$i"} eq "" ) && ( $form->{"partsgroup_$i"} eq "" ) && ( $form->{"partsgroupcode_$i"} eq "" ) ) {
    ...
    if ($rows) {
      ...
      if ( $rows > 1 ) {
        ...
        # bp 2014/11 - we are using $maxpricedecimals instead of $priceprecision and $costprecision
        ...
        for (qw(sellprice listprice)) { $form->{"${_}_$i"} = $form->format_amount( \%myconfig, $form->{"${_}_$i"}, $form->{maxpricedecimals} ) }
...
sub transactions {
  ...
  @columns = $form->sort_columns(
    "transdate",   "reqdate", "transportdate", "id",            "$ordnumber", "ponumber", "name",   "$form->{vc}number",
    "description", "memo",    "notes",         "intnotes",      "netamount",  "tax",      "amount", "curr",
    "employee",    "manager", "warehouse",     "shippingpoint", "shipvia",    "waybill",  "open",   "closed"
  );
...
|;
  ...
  foreach $ref ( @{ $form->{OE} } ) {
    ...
    # get defaultprecision and precision for the $ref->{curr} (item line)
    my $dbh              = $form->dbconnect( \%myconfig );
    my %res              = $form->get_defaults( $dbh, \@{ ['precision'] } );
    my $defaultprecision = $res{"precision"};

    my $precision = $form->get_precision( \%myconfig, $ref->{curr} );

    # bp 2011/02/28 - show foreign currencies values with correct rounding

    # in addition we need the local currency
    my $res = $form->get_currencies( $dbh, \%myconfig );
    my @curr = split /:/, $res;
    my $defaultcurrency = $curr[0];

    if ( $form->{l_curr} ) {

      for (qw(netamount amount)) {
        $ref->{"fx_$_"} = $ref->{$_} if ( $ref->{curr} ne $defaultcurrency );
      }

      # convert into default currency and format with defaultprecision
      for (qw(netamount amount)) {
        $ref->{"$_"} = $form->round_amount( $ref->{$_} *= $ref->{exchangerate}, $defaultprecision );

        # bp 2014/11 round with the precision for the currency
        $ref->{"fx_$_"} = $form->round_amount( $ref->{"fx_$_"}, $precision );
      }

      $ref->{tax}    = $ref->{amount} - $ref->{netamount};
      $ref->{fx_tax} = $ref->{fx_amount} - $ref->{fx_netamount};

      # tax column -check: amount - tax = netamount must be true!
      $column_data{tax}    = "<td align=right>" . $form->format_amount( \%myconfig, $ref->{amount} - $ref->{netamount},       $defaultprecision, "&nbsp;" ) . "</td>";
      $column_data{fx_tax} = "<td align=right>" . $form->format_amount( \%myconfig, $ref->{fx_amount} - $ref->{fx_netamount}, $precision,        "&nbsp;" ) . "</td>";

      # format netamount and amount
      for (qw(netamount amount)) {
        $column_data{"$_"}    = "<td align=right>" . $form->format_amount( \%myconfig, $ref->{"$_"},    $defaultprecision, "&nbsp;" ) . "</td>";
        $column_data{"fx_$_"} = "<td align=right>" . $form->format_amount( \%myconfig, $ref->{"fx_$_"}, $precision,        "&nbsp;" ) . "</td>";
      }
...
"<td><a href=$form->{script}?path=$form->{path}&action=$action&type=$form->{type}&id=$ref->{id}&warehouse=$warehouse&vc=$form->{vc}&login=$form->{login}&callback=$callback>$ref->{$ordnumber}</a></td>";
...
|;
 ...
 <tr class=listtotal>|;
  ...
  $column_data{netamount} = "<th class=listtotal align=right>" . $form->format_amount( \%myconfig, $totalnetamount,                $defaultprecision, "&nbsp;" ) . "</th>";
  $column_data{tax}       = "<th class=listtotal align=right>" . $form->format_amount( \%myconfig, $totalamount - $totalnetamount, $defaultprecision, "&nbsp;" ) . "</th>";
  $column_data{amount}    = "<th class=listtotal align=right>" . $form->format_amount( \%myconfig, $totalamount,                   $defaultprecision, "&nbsp;" ) . "</th>";
...
sub save {
  ...
  # bp 2014/10 if transport data has been entered we also need a transport date
  if ( $form->{transportname} && $form->{transportdate} eq '' ) {
    if ( $form->{type} eq 'sales_order' ) {
      $msg = $locale->text('Transport Date missing!');
    }
    else {
      $msg = $locale->text('Pickup Date missing!');
    }
    $form->isblank( "transportdate", $msg );
  }
...
sub invoice {
  ...
  $defaultprecision = $form->get_precision( \%myconfig, $form->{defaultcurrency} );
  $form->{creditremaining} -= $form->format_amount(\%myconfig, $form->{oldinvtotal} - $form->{ordtotal}, $defaultprecision );
...
sub display_ship_receive {
 ...
 <th align=right>| . $locale->text('Warehouse') . qq|</th>
 <td><select name=warehouse>|
    . $form->select_option( $form->{selectwarehouse}, $form->{warehouse}, 1 ) . qq|</select>
 </td>
 </tr>
| if $form->{selectwarehouse};
...
<table width=100%>
 ...
 <td colspan=3>$form->{$form->{vc}}</td>|
    . $form->hide_form( "$form->{vc}", "$form->{vc}_id" ) . qq|
 </tr>
 $department
 <tr>
 <th align=right>| . $locale->text('Shipping Point') . qq|</th>
...
sub generate_purchase_orders {
  ...
  foreach $parts_id ( sort { $form->{orderitems}{$a}{ndx} <=> $form->{orderitems}{$b}{ndx} } keys %{ $form->{orderitems} } ) {
    ...
    $form->{"curr_$i"}        = $form->{defaultcurrencyc};
    $form->{"description_$i"} = $form->{orderitems}{$parts_id}{description};

    $form->{"lastcost_$i"} = $form->format_amount( \%myconfig, $form->{orderitems}{$parts_id}{lastcost}, $form->{maxpricedecimals} );
    ...
    if ( exists $form->{orderitems}{$parts_id}{"parts$form->{vc}"} ) {
      ...
      foreach $id (
        sort {
          $form->{orderitems}{$parts_id}{"parts$form->{vc}"}{$a}{lastcost} *
            $form->{ $form->{orderitems}{$parts_id}{"parts$form->{vc}"}{$a}{curr} } <=> $form->{orderitems}{$parts_id}{"parts$form->{vc}"}{$b}{lastcost} *
            $form->{ $form->{orderitems}{$parts_id}{"parts$form->{vc}"}{$b}{curr} }
        ...
        )
      {
        ...
        $form->{"lastcost_$i"} = $form->format_amount( \%myconfig, $form->{orderitems}{$parts_id}{"parts$form->{vc}"}{$id}{lastcost}, $form->{maxpricedecimals} );
        $form->{"leadtime_$i"} = $form->format_amount( \%myconfig, $form->{orderitems}{$parts_id}{"parts$form->{vc}"}{$id}{leadtime} );
        $form->{"fx_$i"} =
          $form->format_amount( \%myconfig, $form->{orderitems}{$parts_id}{"parts$form->{vc}"}{$id}{lastcost} * $form->{ $form->{orderitems}{$parts_id}{"parts$form->{vc}"}{$id}{curr} },
          $form->{precision} );
