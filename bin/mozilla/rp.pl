...
|;
    ...
    if ( $form->{reportcode} eq "projects" ) {
	  ...
	  <td colspan=3><input name=fromdate size=11 class=date title="$myconfig{dateformat}" value=$form->{fromdate}> <b>|
          . $locale->text('To')
          . qq|</b> <input name=todate size=11 class=date title="$myconfig{dateformat}"></td>
	</tr>
	$selectfrom
      </table>
    </td>
...
|;
	...
	<tr>
	  <th align=right>| . $locale->text('From') . qq|</th>
	  <td colspan=3><input name=comparefromdate size=11 class=date title="$myconfig{dateformat}"> <b>|
          . $locale->text('To')
          . qq|</b> <input name=comparetodate size=11 class=date title="$myconfig{dateformat}"></td>
	</tr>
|;
...
sub generate_income_statement {
    ...
    for (qw(path login accounttype comparefromdate comparemonth compareyear currency decimalplaces fromdate frommonth fromyear interval method todate reportcode reportlogin)) {
...
|;
    ...
    $form->hide_form(
        qw(department projectnumber fromdate todate frommonth fromyear comparefromdate comparemonth compareyear interval currency decimalplaces language_code l_heading l_subtotal l_accno accounttype method)
    );
...
sub generate_balance_sheet {
    ...
    for (qw(path login asofdate asofmonth asofyear compareasofdate compareasofmonth compareasofyear currency decimalplaces accounttype method reportcode reportlogin)) {
        $form->{callback} .= "&$_=$form->{$_}";
    }
