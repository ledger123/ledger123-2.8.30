...
<table width=100%>
  <tr><th class=listtop>$form->{helpref}$form->{title}</th></tr>
  <tr height="5"></tr>
  <tr>
    <td>
      <table>
	  ...
	  <td>
		...
		<td><input name=precision class="inputright" size=5 value="$form->{precision}">
    &nbsp;&nbsp;<span class="strong">|.$locale->text('Max Price Decimals').qq|</span>
    &nbsp;<input name=maxpricedecimals class="inputright" size=1 value="$form->{maxpricedecimals}"></td>
	      </tr>
	      <tr>
...
sub save_defaults {
  ...
  # bp 2014/11 'maxpricedecimals' determines the max decimals of the 'lastcost' and 'sellprice' fields
  #            low price items may be less then the smallest currency unit e.g. < EUR 0.01 or < 1 Jpy
  #            but still we want to limit the decimals to avoid figures like 0.00666666666 EUR
  $form->{maxpricedecimals} = 2 if $form->{maxpricedecimals} eq '';
  $form->error($locale->text('Max Price Decimals must not be smaller than the Precision'))
      if ( $form->{maxpricedecimals} < $form->{precision} );
