...
sub search_employee {
  ...
  push @f, qq|<input name="l_employeenumber" type=checkbox class=checkbox value=Y checked> |
...
<table width=100%>
 ...
 <tr>
   ...
   <td>|
    . $locale->text('From')
    . qq| <input name=startdatefrom size=11 class=date title="$myconfig{dateformat}"> |
    . $locale->text('To')
    . qq| <input name=startdateto size=11 class=date title="$myconfig{dateformat}"></td>
 </tr>
 <tr valign=top>
...
sub list_employees {
  ...
  @columns = $form->sort_columns(
    qw(id employeenumber name address city state zipcode country workphone workfax workmobile homephone homemobile email startdate enddate ssn dob iban bic sales acsrole login notes department warehouse)
  );
  unshift @columns, "ndx";
  ...
  $column_header{employeenumber} =
    qq|<th><a class=listheading href=$href&sort=employeenumber>| . $locale->text('Number') . qq|</a></th>|;
  ...
  $column_header{state} =
    qq|<th><a class=listheading href=$href&sort=state>| . $locale->text('State/Province') . qq|</a></th>|;
  $column_header{zipcode} =
    qq|<th><a class=listheading href=$href&sort=zipcode>| . $locale->text('Zip/Postal Code') . qq|</a></th>|;
  $column_header{country} =
    qq|<th><a class=listheading href=$href&sort=country>| . $locale->text('Country') . qq|</a></th>|;
  $column_header{workphone} =
    qq|<th><a class=listheading href=$href&sort=workphone>| . $locale->text('Work Phone') . qq|</a></th>|;
  $column_header{workfax} =
    qq|<th><a class=listheading href=$href&sort=workfax>| . $locale->text('Work Fax') . qq|</a></th>|;
  $column_header{workmobile} =
    qq|<th><a class=listheading href=$href&sort=workmobile>| . $locale->text('Work Mobile') . qq|</a></th>|;
  $column_header{homephone} =
    qq|<th><a class=listheading href=$href&sort=homephone>| . $locale->text('Home Phone') . qq|</a></th>|;
  $column_header{homemobile} =
    qq|<th><a class=listheading href=$href&sort=homemobile>| . $locale->text('Home Mobile') . qq|</a></th>|;

  $column_header{startdate} =
    qq|<th><a class=listheading href=$href&sort=startdate>| . $locale->text('Startdate') . qq|</a></th>|;
  $column_header{enddate} =
    qq|<th><a class=listheading href=$href&sort=enddate>| . $locale->text('Enddate') . qq|</a></th>|;
  $column_header{notes} = qq|<th><a class=listheading href=$href&sort=notes>| . $locale->text('Notes') . qq|</a></th>|;
  $column_header{acsrole} =
    qq|<th><a class=listheading href=$href&sort=acsrole>| . $locale->text('Role') . qq|</a></th>|;
  ...
  $column_header{department} =
    qq|<th><a class=listheading href=$href&sort=bic>| . $locale->text('Department') . qq|</a></th>|;
  $column_header{warehouse} =
    qq|<th><a class=listheading href=$href&sort=bic>| . $locale->text('Warehouse') . qq|</a></th>|;
...
|;
  ...
  foreach $ref ( @{ $form->{all_employee} } ) {
    ...
    $column_data{name} =
"<td><a href=$form->{script}?action=edit&db=employee&id=$ref->{id}&path=$form->{path}&login=$form->{login}&status=$form->{status}&callback=$callback>$ref->{name}&nbsp;</td>";
...
|;
  ...
  $button{'HR--Employees--Add Employee'}{code} =
    qq|<input class=submit type=submit name=action value="| . $locale->text('Add Employee') . qq|"> |;
...
sub prepare_employee {
  ...
  for (qw(paymentmethod payment ap wage deduction acsrole)) {
...
|;
  ...
  # armaghan 11-apr-2012 added department/warehouse
  $form->hide_form(
    qw(acs payrate_rows wage_rows deduction_rows reference_rows referenceurl status title helpref oldemployeelogin oldemployeepassword company)
  );
  ...
  <td><select name=acsrole>|
        . $form->select_option( $form->{selectacsrole}, $form->{acsrole}, 1 ) . qq|</select></td>
       </tr>
|;
    }
...
|;
  ...
  <td><select name="wage_$i">|
        . $form->select_option( $form->{selectwage}, $form->{"wage_$i"}, 1 ) . qq|</select></td>
       </tr>
|;
    }
    print qq|
...
|;
  ...
  <td><select name="deduction_$i">|
        . $form->select_option( $form->{selectdeduction}, $form->{"deduction_$i"}, 1 ) . qq|</select></td>
  <td><input name="exempt_$i" class="inputright" value=|
        . $form->format_amount( \%myconfig, $form->{"exempt_$i"}, $form->{precision} )
        . qq|></td>
  <td><input name="maximum_$i" class="inputright" value=|
        . $form->format_amount( \%myconfig, $form->{"maximum_$i"}, $form->{precision} )
        . qq|></td>
       </tr>
|;
    }
...
|;
 ...
 <tr valign=top>
  ...
  <td><select name="ap">|
    . $form->select_option( $form->{selectap}, $form->{ap} ) . qq|</select></td>
       </tr>
       <tr>
         <th align=right>| . $locale->text('Payment') . qq|</th>
  <td><select name="payment">|
    . $form->select_option( $form->{selectpayment}, $form->{payment} ) . qq|</select></td>
       </tr>
       $paymentmethod
       <tr>
...
|;
  ...
  for $i ( 1 .. $form->{payrate_rows} ) {
    print qq|
       <tr>
  <td><input name="rate_$i" size=10 class="inputright" value=|
      . $form->format_amount( \%myconfig, $form->{"rate_$i"}, $form->{precision} )
      . qq|></td>
  <td><input name="over_$i" size=10 class="inputright" value=|
      . $form->format_amount( \%myconfig, $form->{"over_$i"} )
      . qq|></td>
       </tr>
|;
  }
...
sub prepare_payroll {
  ...
  for (qw(formname language employee ap payment paymentmethod printer department project)) {
    $form->{"select$_"} = $form->escape( $form->{"select$_"}, 1 );
  }
...
| if $form->{selectpaymentmethod};
  ...
  $project = qq|
    ...
    . $form->select_option( $form->{selectproject}, $form->{project}, 1 ) . qq|</select></td>
      </tr>
| if $form->{selectproject};
  ...
  $department = qq|
      <tr>
        <th align=right nowrap>| . $locale->text('Department') . qq|</th>
        <td><select name=department>|
    . $form->select_option( $form->{selectdepartment}, $form->{department}, 1 ) . qq|</select></td>
      </tr>
| if $form->{selectdepartment};
...
|;
  ...
  $form->hide_form( map { "select$_" }
                    qw(formname language employee ap payment paymentmethod printer department project) );
...
<table width=100%>
  ...
  <td><select name=employee onChange="javascript:document.forms[0].submit()">|
    . $form->select_option( $form->{selectemployee}, $form->{employee}, 1 ) . qq|</select></td>
       </tr>
|;
...
# bp 2015/01 print hidden lines for all deductions
#      $form->hide_form( map { "${_}_$i" } qw(deduction deduction_id deduct) );
    }
     $form->hide_form( map { "${_}_$i" } qw(deduction deduction_id deduct bdeduct) );
  }
...
#      if ( $form->{payrolldeduction}{ $ed->{id} }{employeepays} ) {
        ...
        for $ref ( @{ $form->{all_deductionrate} } ) {
          ...
          if ( $ref->{rate} eq 0 ) {
            ...
            $amtset = 0;
            $form->{"damount_$i"} = $ref->{rate} * $form->{gross};
          }
              ...
              for ( @{ $form->{deduct}{ $ref->{trans_id} } } ) {
                if ( $form->{deduct}{ $ed->{id} }[$j]{trans_id} == $ref->{trans_id} ) {
                  ...
                  if ( $form->{deduct}{ $ed->{id} }[$j]{withholding} ) {
                   $fromwithholding +=
                      $temp{ $form->{deduct}{ $ed->{id} }[$j]{id} } * $form->{deduct}{ $ed->{id} }[$j]{percent};
                  } else {
                    $fromincome +=
                      $temp{ $form->{deduct}{ $ed->{id} }[$j]{id} } * $form->{deduct}{ $ed->{id} }[$j]{percent};
                  }
...
#            }
        ...
        # bep 2015/01 - deduction check (max, age restriction are done for the whole amount
        $dramount = ( $form->{"deduct_$i"} - $fromwithholding - $ed->{exempt} / $form->{payperiod} );
        $dramount = 0 if $dramount < 0;

        # check if amount is over maximum
        if ( $ed->{maximum} ) {
          if ( ( $dramount + $form->{total}{ $ed->{id} } ) > $ed->{maximum} ) {
            $dramount = $ed->{maximum} - $form->{total}{ $ed->{id} };
          }
        }
        ...
        if ( $form->{payrolldeduction}{ $ed->{id} }{fromage} || $form->{payrolldeduction}{ $ed->{id} }{toage} ) {
          ...
          if ( $transdate && $dob ) {
            ...
            if ( $form->{payrolldeduction}{ $ed->{id} }{toage} ) {
              ...
              $transdate = substr( $form->datetonum( \%myconfig, $form->{transdate} ), 0, 4 ) . "0101"
                unless $form->{payrolldeduction}{ $ed->{id} }{agedob};
              $dramount = 0 if $transdate > $d;
            }
          }
        }

        # bp 2015/01 employer's share
        $bamount = $dramount * $form->{payrolldeduction}{ $ed->{id} }{employerpays} * -1;
        $form->{"bdeduct_$i"} = $form->round_amount( $bamount, $form->{precision} );

        # bp 2015/01 employee's share
        $amount = $dramount * $form->{payrolldeduction}{ $ed->{id} }{employeepays};

        $form->{"deduct_$i"} = $form->round_amount( $amount, $form->{precision} );

        # bp 2015/01 store the whole deduction amount (employee + employer)
        $temp{ $ed->{id} } = $dramount;
...
|;
  ...
  if ( @{ $form->{all_project} } ) {
   ...
   <td><select name=projectnumber>|
      . $form->select_option( $form->{selectprojectnumber}, $form->{projectnumber}, 1 ) . qq|</select></td>
        </tr>
|;
  }
...
|;
 ...
 <td>
   <select name=report onChange="ChangeReport();">|
      . $form->select_option( $form->{selectreportform}, undef, 1 )
      . qq|</select>
        </td>
      </tr>
|;
...
<table width=100%>
 ...
 <tr>
   ...
   <td>|
    . $locale->text('From')
    . qq| <input name=transdatefrom size=11 class=date title="$myconfig{dateformat}"> |
    . $locale->text('To')
    . qq| <input name=transdateto size=11 class=date title="$myconfig{dateformat}"></td>
 </tr>
 $selectfrom
 $paymentmethod
 <tr>
...
|;
  ...
  foreach $ref ( @{ $form->{transactions} } ) {
    ...
    for ( @{ $form->{all_wage} } ) {
      $column_data{ $_->{id} } = "<td align=right>"
        . $form->format_amount( \%myconfig, $ref->{ $_->{id} }, $form->{precision}, "&nbsp;" ) . "</td>";
...
"<td><a href=ap.pl?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}&nbsp;</a></td>";
    $column_data{reference} =
"<td><a href=gl.pl?action=edit&id=$ref->{glid}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{reference}&nbsp;</a></td>";
    $column_data{payslip} =
"<td><a href=$form->{script}?action=edit&id=$ref->{id}&db=payroll&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}&nbsp;</a></td>";
...
|;
  ...
  $column_data{paid} = "<th align=right>"
    . $form->format_amount( \%myconfig, $form->{"totalpaid"}, $form->{precision}, "&nbsp;" ) . "</th>";

  for ( @{ $form->{all_wage} } ) {
    $column_data{ $_->{id} } = "<th align=right>"
      . $form->format_amount( \%myconfig, $form->{"total$_->{id}"}, $form->{precision}, "&nbsp;" ) . "</th>";
  }
...
sub payroll_subtotal {
  ...
  $column_data{paid} = "<th align=right>"
    . $form->format_amount( \%myconfig, $form->{"subtotalpaid"}, $form->{precision}, "&nbsp;" ) . "</th>";

  for ( @{ $form->{all_wage} } ) {
    $column_data{ $_->{id} } = "<td align=right>"
      . $form->format_amount( \%myconfig, $form->{"subtotal$_->{id}"}, $form->{precision}, "&nbsp;" ) . "</td>";
...
sub search_deduction {
  ...
  @column_index =
    qw(description rate amount above below basedon employeepays employee_accno employerpays employer_accno);
...
"<td><a href=$form->{script}?action=edit&db=$form->{db}&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{description}</a></td>";
    } else {
...
sub prepare_deduction {
  ...
  # bp 2015/01 enter percentage values
  for (qw(employeepays employerpays)) { $form->{"$_"} *= 100 };
...
<table width=100%>
 ...
 <tr>
   <th align=right nowrap>| . $locale->text('Employee pays') . qq| %</th>
   <td><input name=employeepays class="inputright" size=4 value=|
    . $form->format_amount( \%myconfig, $form->{"employeepays"} ) . qq|>
   <select name=employee_accno>|
    . $form->select_option( $form->{selectaccounts}, $form->{employee_accno} ) . qq|</select></td>
 </tr>
 <tr>
   <th align=right nowrap>| . $locale->text('Employer pays') . qq| %</th>
   <td><input name=employerpays class="inputright" size=4 value=|
    . $form->format_amount( \%myconfig, $form->{employerpays} ) . qq|>
 ...
 <tr>
   ...
   <td><input name=fromage class="inputright" size=4 value=|
    . $form->format_amount( \%myconfig, $form->{fromage} ) . qq|>
   <b>&gt;</b>
   <input name=toage class="inputright" size=2 value=| . $form->format_amount( \%myconfig, $form->{toage} ) . qq|>
...
|;
  ...
  for $i ( 1 .. $form->{rate_rows} ) {
    print qq|
       <tr>
  <td><input name="rate_$i" class="inputright" size=11 value=|
  ...
  <td><input name="amount_$i" class="inputright" size=11 value=|
      ...
      . qq|></td>
  <td><input name="above_$i" class="inputright" size=11 value=|
      . $form->format_amount( \%myconfig, $form->{"above_$i"}, $form->{precision} )
      . qq|></td>
  <td><input name="below_$i" class="inputright" size=11 value=|
      . $form->format_amount( \%myconfig, $form->{"below_$i"}, $form->{precision} )
      . qq|></td>
       </tr>
|;
  }
...
|;
  ...
  <td><input name="percent_$i" class="inputright" size=10 value=|
        . $form->format_amount( \%myconfig, $form->{"percent_$i"} )
        . qq|></td>
       </tr>
      |;
    }
...
|;
  ...
  foreach $ref ( @{ $form->{all_wage} } ) {
    ...
    $column_data{amount} =
      "<td align=right>" . $form->format_amount( \%myconfig, $ref->{amount}, $form->{precision}, "&nbsp;" ) . "</td>";
...
<table width=100%>
 ...
 <tr>
   ...
   <td><input name=amount class=inputright size=10 value=|
    . $form->format_amount( \%myconfig, $form->{amount}, $form->{precision} )
    . qq|></td>
 </tr>
        <tr>
   <th></th>
    <td><input name="exempt" type=checkbox class=checkbox value=1 $checked{exempt}> <b>|
    . $locale->text('Exempt')
    . qq|</b></td>
 </tr>
      </table>
...
sub save_deduction {
  ...
  unless ( $form->{"rate_1"} || $form->{"amount_1"} ) {
    # bp 2015/01 changed error message
    $form->isblank( "rate_1", $locale->text("Enter Rate or Amount!") ) unless $form->{"amount_1"};
    #$form->isblank( "rate_1", $locale->text("Rate missing!") ) unless $form->{"amount_1"};
    # bp 2015/01 this error message will never show
    $form->isblank( "amount_1", $locale->text("Amount missing!") );
  }

  # bp 2015/01 enter rate for different calculaton basis
  if ( $form->{"amount_1"} && $form->{"basedon"} ) {
    $form->isblank( "rate_1", $locale->text("Enter a rate with a Based_on deduction") );
  }

  # bp 2015/01 sum of employeepays and employerpays must be 100%
  if (  $form->{employeepays} + $form->{employerpays} ne 100 ) {
    $form->error( $locale->text("The sum of 'Employee pays' and 'Employer pays' must be 100%") );
  } else {
    for (qw(employeepays employerpays)) { $form->{$_} = $form->{$_} / 100 };
  }
