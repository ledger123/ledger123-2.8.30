...
sub edit {
  ...
  foreach $ref (@{ $form->{GL} }) {
    ...
    #bp 2014/12 valid fxamount and foreign currency -> show fxamount instead of amount
    # old document don't have a valid fxamount and hence $amount will be shown - as before
    if ( $ref->{fxamount} ) {
      if ( $form->{currency} ne $form->{defaultcurrency} )  {
        $ref->{amount} = $ref->{fxamount};
      }
    }
