...
|;
    ...
    $form->hide_form(
        qw(id type printed emailed sort closedto locked oldtransdate oldduedate oldcurrency audittrail recurring checktax creditlimit creditremaining defaultcurrency rowcount oldterms batch batchid batchnumber batchdescription cdt precision remittancevoucher reference_rows referenceurl)
    );
...
|;
        ...
        $column_data{paid} =
          qq|<td align=center><input name="discount_paid" class="inputright" size=11 value=| . $form->format_amount( \%myconfig, $form->{"discount_paid"}, $form->{precision} ) . qq|></td>|;
...
|;
    ...
    $openclosed = qq|
		...
		<td nowrap><input name=closed class=checkbox type=checkbox value=Y $pref{closed_checked}> | . $locale->text('Closed') . qq|</td>
...
|;
    ...
    foreach $ref ( @{ $form->{transactions} } ) {
        ...
        for (qw(department ordnumber ponumber notes intnotes warehouse shippingpoint shipvia waybill employee till source memo description projectnumber address dcn paymentmethod)) {
