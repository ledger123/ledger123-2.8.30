...
sub header {
  ...
  # bp 2014/11 datepicker uses 'y' for the 2-digit year and 'yy' for the 4-digit year
  #            'yyyy' will print the 4-year digit twice like: 20142014-11-01
  my $dtformat = $self->{dateformat};
  if ( $dtformat =~ /yyyy/ ) {
    # date format: yyyy-mm-dd, change to yy-mm-dd
    $dtformat =~ s/^yyyy/yy/g;
  } else {
    # variations of mm/dd/yy and dd/mm/yy, change yy to 'y'
    $dtformat =~ s/yy/y/g;
  }
...
sub format_amount {
  ...
  if ($amount) {
    if ($myconfig->{numberformat}) {
      ...
      # bp 2014/11 suppress trailing zeroes with $dash='SZ'
      } elsif ($dash =~ /SZ/) {
        if ($dec2 eq '' || $dec2 == 0) {
          $amount =~ s/\.0*//g ;
        } else {
            $amount =~ m/^(.*$sep\d*[1-9])(0*)$/;
            $amt = $1;
            $amount = $amt;
       }
        $dash = '';
...
sub process_template {
  ...
  while ($_ = shift) {
    ...
    if (/<%include /) {
      ...
      $var = $_;
      # bp 2014/11 - remove trailing chars, otherwise the html template is printed
      chomp($var);
      $var =~ s/(\s*?<%|%>)//g;
...
#bp 2014/10 - don't delete tmpfiles for debugging
    unlink(<$tmpfile.*>);
  }
...
sub create_links {
  ...
  # bp 2014/11 added maxpricedecimals
  my @df = qw(closedto revtrans weightunit cdt precision maxpricedecimals roundchange cashovershort_accno_id referenceurl);
  ...
  if ($self->{id}) {
    ...
    # bp 2014 for correct conversion of foreign currency docs we need the precision of the document currency
    @curr = split /:/, $this->{currencies};
    $this->{defaultcurrency} = $curr[0];
    chomp $this->{defaultcurrency};
    my $precision = 1;
    if ( $ref->{currency} ne $this->{defaultcurrency} ) {
      $precision = $self->get_precision($myconfig, $ref->{currency} );
     }
    for (qw(oldinvtotal oldtotalpaid)) { $ref->{$_} = $self->round_amount($ref->{$_} / $ref->{exchangerate}, $precision) }
    #
    ...
    # bp 2014/11 added y.fxamount to field selection (holds paid amount in payment currency)
...
#    $mm--;
    @t = (1,0,0,$dd,$mm,$yy);
#  }
  # bp 2014/11 no 'else' -> then we overwrite $date with the current date
  } else {
    ...
    # the year is only 4 digits if the dateformat is yyyy-mm-dd
    $yy = substr($yy,2,2) if $myconfig->{dateformat} !~ /^yyyy/;
  }
