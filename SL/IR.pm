 ...
 WHERE id = ?|;
  ...
  for my $item (@sortlist) {
    ...
    if ( $form->{"qty_$i"} ) {
      ...
      # bp 2014/11 maxpricedecimals, new config field
#      my $dbh = $form->dbconnect($myconfig);
#      %res = $form->get_defaults( $dbh, \@{ ['maxpricedecimals'] } );


      my $maxpricedecimals = $form->{maxpricedecimals};
      ...
      my $discount = $form->round_amount( $sellprice * $form->parse_amount( $myconfig, $form->{"discount_$i"} ) / 100, $form->{maxpricedecimals} );
      ...
      push( @{ $form->{netprice} }, ( $form->{"netprice_$i"} ) ? $form->format_amount( $myconfig, $form->{"netprice_$i"}, $form->{maxpricedecimals} ) : " " );

      $discount = ($discount) ? $form->format_amount( $myconfig, $discount * -1, $form->{maxpricedecimals} ) : " ";
...
sub post_invoice {
  ...
  # bp 2014/11 let's be clear about precisions
  $defaultprecision = $defaults{precision};
  $form->{precision} = $form->get_precision( $myconfig, $form->{currency} );

# bp 2014/12 should already be set
#  $form->{maxpricedecimals} = $defaults{maxpricedecimals};
 ...
 VALUES ($form->{id}, $form->{department_id})|;
  ...
  for $i ( 1 .. $form->{rowcount} ) {
    ...
    if ( $form->{"qty_$i"} ) {
      ...
      $discount = $form->round_amount( $fxsellprice * $discount / 100, $form->{maxpricedecimals} );
      ...
      $fxdiff += $form->round_amount( $amount - $linetotal, $defaultprecision );
      ...
      $grossamount = $form->round_amount( $linetotal, $defaultprecision );

      if ( $form->{taxincluded} ) {
        $amount = $form->round_amount( $tax, $defaultprecision );
        $linetotal -= $form->round_amount( $tax - $diff, $defaultprecision );
        $diff = ( $amount - $tax );
      }

      $amount = $form->round_amount( $linetotal, $defaultprecision );

      $form->{"sellprice_$i"} = $form->round_amount($form->{"sellprice_$i"} * $form->{exchangerate}, $form->{maxpricedecimals});


      # bp 2014/11 calculate invoice value in home currency using the entered exchange rate
      $calcinvvalue += $fxsellprice * $form->{"qty_$i"};
 ...
 WHERE id = $form->{"id_$i"}|;
  ...
  # bp 2014/11 check if $form->{paid} is in foreign currency
 ...
 $amount, '$form->{transdate}')|;
  ...
  for $i ( 1 .. $form->{paidaccounts} ) {
    ...
    if ( $form->{"paid_$i"} ) {
      ...
      # record AP
      $amount = ( $form->round_amount( $form->{"paid_$i"} * $form->{exchangerate}, $defaultprecision ) ) * -1;
 ...
 $paymentmethod_id)|;
      ...
      $amount = $form->round_amount( $form->{"paid_$i"} * $form->{"exchangerate_$i"} - $form->{"paid_$i"}, $defaultprecision );
 ...
 '$approved', $voucherid)|;
      ...
      $amount =
        $form->round_amount(
        $form->round_amount( $form->{"paid_$i"} * $form->{exchangerate}, $defaultprecision ) - $form->round_amount( $form->{"paid_$i"} * $form->{"exchangerate_$i"}, $defaultprecision ),
        $defaultprecision );
 ...
 '$approved', $voucherid)|;
  ...
  # bp 2014/11 when posting the invoice the value in foreign currency is calculated using the value in home
  # currency and the exchange rate (stored in ap). Due to rounding - in particular with Japanese Yen (no decimals)
  # this can easily lead to differences of 1 or 2 Yen.
  # We minimize this by calculating the effective exchange rate for the invoice total.
  # i.e. (total amount in Jpy) / (total amount in foreign currency) - with a precision of 10 digits

  $calcinvvalue = $form->parse_amount( $myconfig, $calcinvvalue );
  my $calcexrate = 0;
  $calcexrate = ( $invamount / $calcinvvalue );


  # bp 2014/11 round the calcuated exchange rate to 10 decimals
  my $calcexrate = $form->round_amount( $calcexrate,  10 );
 ...
 WHERE i.parts_id = ?|;
  ...
  for my $item ( keys %updparts ) {
    ...
    $avgcost = 0;
    if ($qty) {
      $avgcost = $form->round_amount( $cost / $qty, $defaultprecision );
    }
...
sub reverse_invoice {
 ...
 AND i.trans_id = $form->{id}|;
  ...
  %defaults = $form->get_defaults($dbh, \@{['maxpricedecimals']});
  $form->{maxpricedecimals} = $defaults{maxpricedecimals};
 ...
 ORDER BY trans_id DESC|;
        ...
        while ( $pref = $sth->fetchrow_hashref(NAME_lc) ) {
          ...
          $amount = $form->round_amount( $ref->{sellprice} * $qty, $form->{maxpricedecimals} );
...
#  %res = $form->get_defaults($dbh, \@{['maxpricedecimals']});
  my $maxpricedecimals = $form->{maxpricedecimals};
    ...
    while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
      ...
      # price matrix
      $ref->{sellprice} = $form->round_amount( $ref->{fxsellprice} * $form->{ $form->{currency} }, $form->{maxpricedecimals} );
      &price_matrix( $pmh, $ref, $form->{maxpricedecimals}, $form );
...
#    &price_matrix( $pmh, $ref, $maxpricedecimals, $form );
...
sub price_matrix {
  my ( $pmh, $ref, $form ) = @_;
#  my ( $pmh, $ref, $maxpricedecimals, $form ) = @_;
  ...
  if ( $mref->{lastcost} ) {
    ...
    # do a conversion
    $ref->{sellprice} = $form->round_amount( $mref->{lastcost} * $form->{ $mref->{curr} }, $form->{maxpricedecimals} );
  }
  $pmh->finish;
