...
sub import_order {
    ...
    for (
        qw(ordnumber quonumber ponumber transdate name reqdate shippingpoint shipvia waybill terms notes intnotes curr exchangerate language_code taxincluded shiptoname shiptoaddress1 shiptoaddress2 shiptocity shiptostate shiptozipcode shiptocountry shiptocontact shiptophone shiptofax shiptoemail)
      )
    {
        $form->{$_} = $form->{"${_}_1"};
    }
...
sub gl {
    ...
    $query = qq|SELECT id FROM project WHERE projectnumber = ?|;
    my $pth = $dbh->prepare($query) || $form->dberror($query);
