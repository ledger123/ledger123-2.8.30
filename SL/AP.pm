...
sub post_transaction {
  ...
  carp("precision: $form->{precision} fxprec: $form->{fxprecision} \n");
  ...
  foreach $accno (@taxaccounts) {
    ...
    # bp 2014/11 1 posting for the whole amount
    ...
    carp("precision tax conversion: $form->{precision} \n");
  ...
  for $i ( 1 .. $form->{rowcount} ) {
    ...
    if ( $amount{fxamount}{$i} ) {
      ...
      # bp 2014/11 one posting for the whole amount
      ...
      carp("amount: $amount{amount}{$i} - fxamount: $amount{fxamount}{$i} \n");
      ...
      carp("invnetamt: $invnetamount - fxinvnet: $fxinvnetamount \n");
  ...
  for $i ( 1 .. $form->{paidaccounts} ) {
    ...
    if ($fxamount) {
      ...
      carp("payment: fxamt: $fxamount - amtpaid: $paidamount \n");
  ...
  carp("fxinvamt: $fxinvamount - invamt: $invamount - paid: $paid - table: $table \n");
  ...
  carp("checking id in table: $table \n");
 ...
 WHERE c.accno = '$paymentaccno'|;
  ...
  carp("AA.pm - update $table - fxinvamout: $fxinvamount \n");
 ...
 VALUES ($form->{batchid}, $form->{id}, $form->{id}, |
  ...
  foreach $ref ( @{ $form->{acc_trans}{lineitems} } ) {
    ...
    carp("AA.pm insert into acc_trans id: $form->{id} - amount: $ref->{amount} - prec $form->{precision} \n");
 ...
 '$ref->{fx_transaction}', '$approved')|;
  ...
  carp("ar/ap: record invadmoun: $invamount \n");
 ...
 $form->{id}, $voucherid, | . $dbh->quote( $form->{voucher}{payment}{$voucherid}{vouchernumber} ) . qq|)|;
      ...
      if ($arap) {
        ...
        carp("add ar/ap for paid amounts \n");
 ...
 '$approved', $voucherid)|;
      ...
      if ($amount) {
        ...
        carp("add acc_trans for payment: $amount \n");
 ...
 $cleared, '$approved', $voucherid, $paymentid)|;
        ...
        carp("AA.pm - insert record into payment table - ap amt in EUR: $fxinvamount \n");
 ...
 $paymentmethod_id)|;
  ...
  # $amount = $form->round_amount(($form->round_amount($paid{fxamount}{$i} * $form->{exchangerate}, $form->{precision});
          ...
          if ($amount) {
            ...
            carp("AA.pm exrate gain/loss: amt: $amount \n");
