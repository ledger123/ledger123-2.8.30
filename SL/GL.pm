...
sub post_transaction {
  ...
  # bp 2014/12 ensure that we have the right precision (precision of the document currency)
  ...
  for $i (1 .. $form->{rowcount}) {
    ...
    if ($form->{"fx_transaction_$i"} *= 1) {
      ...
      # bp 2014/12 we do one posting for the whole amount
    $fxamount = 0;
    ...
    # bp 2014/12 added column fxamount (fx value in fx currency)
    if ($amount || $form->{"source_$i"} || $form->{"memo_$i"} || ($project_id ne 'NULL')) {
      $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount, fxamount, transdate,
		  ...
		  ($form->{id}, (SELECT id
				 FROM chart
				 WHERE accno = '$accno'),
		   $amount, $fxamount, '$form->{transdate}', |.
...
sub transactions {
  ...
  # bp 2014/12 retrieve new column fxamount
  my $query = qq|SELECT g.id, 'gl' AS type, $false AS invoice, g.reference,
                 g.description, ac.transdate, ac.source,
		 ac.amount, ac.fxamount, c.accno, c.gifi_accno, g.notes, c.link,
	...
	UNION ALL
		 ...
		 ac.amount, ac.fxamount, c.accno, c.gifi_accno, a.notes, c.link,
