...
sub get_openvc {
 ...
 ORDER BY $sortorder|;
  ...
  while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
    ...
    if ( $form->{vc} eq 'vendor' ) {
      $ref->{fxdue} =
        $form->round_amount( ( $ref->{amount} - $ref->{paid} ) / $ref->{exchangerate}, $form->{precision} );
...
sub get_openinvoices {
 ...
 AND ac.trans_id = ?|;
  ...
  foreach $ref (@transactions) {
    if ( $form->{vc} eq 'vendor' ) {
      if ( $ref->{threshold} > 0 ) {
        $total{ $ref->{"$form->{vc}_id"} } =
          $form->round_amount( $total{ $ref->{"$form->{vc}_id"} }, $form->{precision} );
...
sub post_payment {
 ...
 AND trans_id = $id|;
      ...
      # update arap
      $form->update_balance( $dbh, $form->{arap}, 'paid', qq|id = $id|,
 ...
 VALUES ($form->{batchid}, ?, $voucherid, |
  ...
  for my $i ( 1 .. $form->{rowcount} ) {
    ...
    for (qw(paid discount due)) { $form->{"${_}_$i"} = $form->parse_amount( $myconfig, $form->{"${_}_$i"} ) }

    # bp 2014/12 - check outstanding amount - don't overpay
    my $out = $form->{"due_$i"} - $form->{"discount_$i"};
    if ( $form->{"paid_$i"} > $out ) { $form->{"paid_$i"} = $out; };
    ...
    if ( $form->{"checked_$i"} ) {
      ...
      # bp 2014/12 check outstanding amount
      $paymentamount -= $form->parse_amount( $myconfig, $form->{"paid_$i"} );

      $ath->execute( $form->{"id_$i"} ) || $form->dberror;
      ($arap) = $ath->fetchrow_array;
      $ath->finish;

      # bp 2011/14 use invoice exchange rate
      $amount =
        $form->round_amount( $form->{"paid_$i"} * $trans{ $form->{"id_$i"} }{exchangerate}, $form->{precision} );

      # add AR/AP
      $query = qq|INSERT INTO acc_trans (trans_id, chart_id, transdate, amount, approved, vr_id)
              VALUES ($form->{"id_$i"}, $arap, '$form->{datepaid}', $amount * $ml, '$approved', $voucherid)|;
      $dbh->do($query) || $form->dberror($query);

      # bp 2014/11 payment amount - use payment exchange rate
      $amount = $form->round_amount( $form->{"paid_$i"} * $form->{exchangerate}, $form->{precision} );

      $query = qq|INSERT INTO acc_trans (trans_id, chart_id, transdate,
                            amount, source, memo, approved, vr_id, id)
              VALUES ($form->{"id_$i"},
                        (SELECT id FROM chart WHERE accno = '$paymentaccno'),
                            '$form->{datepaid}', $amount * $ml * -1, |
                            . $dbh->quote( $form->{source} ) . qq|, | . $dbh->quote( $form->{memo} ) . qq|, '$approved',
                            $voucherid, $paymentid)|;
      $dbh->do($query) || $form->dberror($query);

      # bp 2014/11 we store the amount paid in table payment
      my $amtpaid = $form->round_amount( $form->{"paid_$i"}, $form->{fxprecision} );

      $query = qq|INSERT INTO payment (id, trans_id, exchangerate,paymentmethod_id, fxamount)
              VALUES ($paymentid, $form->{"id_$i"}, $form->{exchangerate}, $paymentmethod_id, $amtpaid)|;
      $dbh->do($query) || $form->dberror($query);

      # gain/loss
      $amount = $form->round_amount(
                                   (
                                     $form->round_amount( $form->{"paid_$i"} * $trans{ $form->{"id_$i"} }{exchangerate},
      ...
      if ($amount) {
        ...
        $query = qq|INSERT INTO acc_trans (trans_id, chart_id, transdate,
                ...
                VALUES ($form->{"id_$i"}, $accno_id,
                           '$form->{datepaid}', $amount, '1', '$approved', $voucherid)|;
        $dbh->do($query) || $form->dberror($query);
      }
      ...
      if ( $form->{"discount_$i"} ) {
        ...
        # bp 2014 cdt is based on the net amount
        ...
        for ( keys %cdt ) {
          ...
          # add AR/AP - inv exchange rate
          $amount = $form->round_amount( $cdt{$_} * $trans{ $form->{"id_$i"} }{exchangerate}, $form->{precision} );

          $query = qq|INSERT INTO acc_trans (trans_id, chart_id, transdate, amount, approved, vr_id, id)
                 VALUES ($form->{"id_$i"}, $arap, '$form->{datepaid}', $amount * $ml,
                            '$approved', $voucherid, $form->{"id_$i"})|;
          $dbh->do($query) || $form->dberror($query);

          my $amtcdt = $form->round_amount( $cdt{$_}, $form->{fxprecision} );


          # bp 2014/11 add cash discount in payment currency and exchange rate to the payment table
          $query = qq|INSERT INTO payment (id, trans_id, exchangerate, paymentmethod_id, fxamount)
                 VALUES ($paymentid+1 , $form->{"id_$i"}, $form->{exchangerate},
                        (select id FROM chart WHERE accno = '$_'), $amtcdt )|;
          $dbh->do($query) || $form->dberror($query);

          # bp 2014/11 total discount in home currency, payment exrate
          $amount = $form->round_amount( $cdt{$_} * ( $form->{exchangerate} ) * $ml * -1, $form->{precision} );


          # post discount to accounting table
          # bp 2014/11 set id = $payment+1 and fx_transaction = false
          $paymentid++;
          $query = qq|INSERT INTO acc_trans (trans_id, chart_id, transdate,
                     amount, fx_transaction, source, approved, vr_id, id)
                 VALUES ($form->{"id_$i"},
                     (SELECT id FROM chart WHERE accno = '$_'),
 '                      $form->{datepaid}', $amount, '0', |
                        . $dbh->quote( $form->{source} ) . qq|, '$approved', $voucherid, $paymentid)|;
          $dbh->do($query) || $form->dberror($query);

          # gain/loss
          $amount = $form->round_amount(
                                         (
                                           $form->round_amount( $cdt{$_} * $trans{ $form->{"id_$i"} }{exchangerate},
                                                                $form->{precision} ) - $form->round_amount(
                                                                                       $cdt{$_} * $form->{exchangerate},
                                                                                       $form->{precision}
                                                                )
                                         ) * $ml * -1,
                                         $form->{precision} );
          ...
          if ($amount) {
            ...
            $query = qq|INSERT INTO acc_trans (trans_id, chart_id, transdate,
                   ...
                   VALUES ($form->{"id_$i"}, $accno_id,
                              '$form->{datepaid}', $amount, '1', '$approved', $voucherid, $form->{"id_$i"})|;
            $dbh->do($query) || $form->dberror($query);
          }
        }
      }
      ...
      $form->{"paid_$i"} =
        $form->round_amount( $form->{"paid_$i"} * $trans{ $form->{"id_$i"} }{exchangerate}, $form->{precision} );
      $form->{"discount_$i"} =
        $form->round_amount( $form->{"discount_$i"} * $trans{ $form->{"id_$i"} }{exchangerate},
                             $form->{precision} );

      # unlock arap
      $pth->finish;

      $amount = $form->round_amount( $trans{ $form->{"id_$i"} }{paid} + $form->{"paid_$i"} + $form->{"discount_$i"},
                                     $form->{precision} );
      ...
      $query = qq|UPDATE $form->{arap} set
                   amount = $trans{$form->{"id_$i"}}{amount}, paid = $amount, datepaid = '$form->{datepaid}',
                   bank_id = (SELECT id FROM chart WHERE accno = '$paymentaccno'), paymentmethod_id = $paymentmethod_id
              WHERE id = $form->{"id_$i"}|;
                $dbh->do($query) || $form->dberror($query);
      ...
      if ( $form->{batch} ) {
        ...
        # update batch
        $form->update_balance( $dbh, 'br', 'amount', qq|id = $form->{batchid}|, $amount );
      }
    }
  }
  ...
  # bp 2014/12 must round in payment currency precision
  if ( $form->round_amount( $paymentamount, $form->{fxprecision} ) ) {
#  if ( $form->round_amount( $paymentamount, $form->{precision} ) ) {

    my $op = $form->round_amount( $paymentamount, $form->{fxprecision} );
#    my $op = $form->round_amount( $paymentamount, $form->{precision} );
