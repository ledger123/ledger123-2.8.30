...
sub post_invoice {
  ...
  foreach $i (1 .. $form->{rowcount}) {
    ...
    $ml = -1;
      ...
      if ($form->{taxincluded}) {
        $taxrnd = $form->round_amount($tax, $defaultprecision);
        $diff = ($taxrnd - $tax);
        $linetotal -= $form->round_amount($tax - $diff, $defaultprecision);
      }
      
      # add linetotal to income - bp: already rounded 
      $amount = $linetotal;
      ...
      $form->{"sellprice_$i"} = $form->round_amount($form->{"sellprice_$i"} * $form->{exchangerate}, $defaultprecision);
  ...
  $amount = 0;
  for (split / /, $form->{taxaccounts}) { $amount += $form->{acc_trans}{$form->{id}}{$_}{amount} = $form->round_amount($form->{acc_trans}{$form->{id}}{$_}{amount}, $defaultprecision) }
  ...
  for $i (1 .. $form->{paidaccounts}) {
    ...
    $dbh->do($query) || $form->dberror($query);
      ...
      # gain/loss
      $amount = $form->round_amount(($form->round_amount($form->{"paid_$i"} * $form->{exchangerate}, $defaultprecision) - $form->round_amount($form->{"paid_$i"} * $form->{"exchangerate_$i"}, $defaultprecision)) * -1, $defaultprecision);
