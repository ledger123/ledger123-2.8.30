 ...
 WHERE id = ?|;
  ...
  for my $item (@sortlist) {
    ...
    if ( $form->{groupprojectnumber} || $form->{grouppartsgroup} ) {
      if ( $item->[1] ne $sameitem ) {
        ...
        if ($ok) {
          ...
          for (
            qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sell sellprice listprice netprice discount discountrate linetotal itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode barcode)
            )
          {
            push( @{ $form->{$_} }, "" );
          }
    ...
    if ( $form->{"qty_$i"} ) {
      ...
      for (
        qw(sku serialnumber ordernumber customerponumber bin description unit deliverydate sell sellprice listprice itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode barcode)
        )
      {
      ...
      my $discount = $form->round_amount( $sellprice * $form->parse_amount( $myconfig, $form->{"discount_$i"} ) / 100, $maxpricedecimals );
      ...
      push( @{ $form->{netprice} }, ( $form->{"netprice_$i"} ) ? $form->format_amount( $myconfig, $form->{"netprice_$i"}, $maxpricedecimals ) : " " );

      $discount = ($discount) ? $form->format_amount( $myconfig, $discount * -1, $maxpricedecimals ) : " ";
      ...
      if ($subtotal) {
        if ( $j < $k ) {
          ...
          if ( $sortlist[$j]->[1] ne $sameitem ) {
            ...
            for (
              qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sell sellprice listprice netprice discount discountrate itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode barcode)
              )
            {
              push( @{ $form->{$_} }, "" );
            }
 ...
 $sortorder|;
  ...
  while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
    ...
    if ( $form->{grouppartsgroup} && $ref->{partsgroup} ne $sm ) {
      for (
        qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber unit qty ship bin deliverydate projectnumber sellprice listprice netprice discount discountrate linetotal itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode barcode)
        )
      {
        push( @{ $form->{$_} }, "" );
      }
    ...
    if ( $form->{stagger} ) {
      ...
      for (
        qw(taxrates runningnumber ship serialnumber ordernumber customerponumber reqdate projectnumber sellprice listprice netprice discount discountrate linetotal itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode barcode)
        )
      {
        push( @{ $form->{$_} }, "" );
      }
...
sub post_invoice {
  ...
  my %defaults = $form->get_defaults( $dbh, \@{ [ 'fx%_accno_id', 'cdt', 'precision', 'maxpricedecimals' ] } );
  $defaultprecision = $defaults{precision};
  $maxpricedecimals = $defaults{maxpricedecimals};
 ...
 VALUES ($form->{id}, $form->{department_id})|;
  ...
  for $i ( 1 .. $form->{rowcount} ) {
    ...
    if ( $form->{"qty_$i"} ) {
      ...
      $discount = $form->round_amount( $fxsellprice * $discount / 100, $maxpricedecimals );
      ...
      # bp - value in home currency must be rounded using the default precision!
      my $linetotal = $form->round_amount( $amount, $defaultprecision );
      ...
      # bp value in domestic currency must be rounded with the defaultprecision
      $form->{"sellprice_$i"} = $form->round_amount( $form->{"sellprice_$i"} * $form->{exchangerate}, $maxpricedecimals );
 ...
 WHERE id = $id|;
      ...
      for (@taxaccounts) {
        ...
        # bp 2014/11 round the tax amount
        $taxamount = $form->round_amount($taxamount, $defaultprecision);
        if ( $taxamount != 0 ) {
 ...
 VALUES ($form->{id}, $id, (SELECT id FROM chart WHERE accno='$_'), $taxamount)|;
      ...
      # armaghan - update inventory at warehouses
 ...
 WHERE id = $form->{"id_$i"}|;
  ...
  # add lineitems + tax -> account postings are in home currency!
  $amount      = 0;
  $grossamount = 0;
  $fxamount    = 0;
  ...
  for ( split / /, $form->{taxaccounts} ) {
    ...
    $amount += $form->{acc_trans}{ $form->{id} }{$_}{amount} = $form->round_amount( $form->{acc_trans}{ $form->{id} }{$_}{amount}, $defaultprecision );
  ...
  if ( $form->{taxincluded} ) {
    ...
    $diff = $form->round_amount( $grossamount - $invamount, $defaultprecision );
    $invamount += $diff;
  }
  $fxdiff = 0 if $form->{rowcount} == 2;

  # $fxdiff = $form->round_amount($fxdiff, $form->{precision});
  $fxdiff = $form->round_amount( $fxdiff, $defaultprecision );
  $invnetamount += $fxdiff;
  $invamount    += $fxdiff;

  $fxtax = $form->round_amount( $fxtax, $defaultprecision );
  ...
  # armaghan - code block moved to prevent formatting issue
  $form->{paid} = 0;
  for $i ( 1 .. $form->{paidaccounts} ) {
    if ( $form->{"paid_$i"} ) {
  ...
  if ( $form->round_amount( $form->{paid} - $fxamount + $fxtax, $defaultprecision ) == 0 ) {
    $form->{paid} = $invamount;
  }
  else {

    # bp 2014 - assume std currency JPY, document curr USD. $form->precision=2 will result in JPY with 2 decimals,
    $form->{paid} = $form->round_amount( $form->{paid} * $form->{exchangerate}, $defaultprecision );
 ...
 $form->{receivables}, '$form->{transdate}')|;
  ...
  if ( $form->{"paid_$i"} && $defaults{cdt} ) {
    ...
    for ( split / /, $form->{taxaccounts} ) {
      ...
      $tax += $roundamount = $form->round_amount( $amount, $defaultprecision );
    ...
    $diff = $form->round_amount( $diff, $defaultprecision );
    if ( $diff != 0 ) {
    ...
    foreach $ref ( @{ $form->{acc_trans}{taxes} } ) {
      $ref->{amount} = $form->round_amount( $ref->{amount}, $defaultprecision );
      if ( $ref->{amount} ) {
        $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount,
 transdate, id)
 VALUES ($form->{id},
 ...
 $ref->{id})|;
  ...
  foreach my $trans_id ( keys %{ $form->{acc_trans} } ) {
    foreach $accno ( keys %{ $form->{acc_trans}{$trans_id} } ) {
      $amount = $form->round_amount( $form->{acc_trans}{$trans_id}{$accno}{amount}, $defaultprecision );
      if ($amount) {
        $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount,
 transdate)
 ...
 $amount, '$form->{transdate}')|;
  ...
  for $i ( 1 .. $form->{paidaccounts} ) {
    ...
    if ( $form->{"paid_$i"} ) {
      ...
      $amount = $form->round_amount( $form->{"paid_$i"} * $form->{exchangerate}, $defaultprecision );
 ...
 $paymentmethod_id)|;
      ...
      # exchangerate difference - bp 2014/11 TODO check - this looks strange
      # $amount = $form->round_amount(
      # ($form->round_amount($form->{"paid_$i"} * $form->{"exchangerate_$i"} - $form->{"paid_$i"}, $form->{precision})) * -1, $form->{precision});

      # bp 2014/11 but in any case amount has to be in home currency
      $amount = $form->round_amount( ( $form->round_amount( $form->{"paid_$i"} * $form->{"exchangerate_$i"} - $form->{"paid_$i"}, $defaultprecision ) ) * -1, $defaultprecision );
 ...
 '$approved', $voucherid)|;
      ...
      # gain/loss
      $amount = $form->round_amount(
        ( $form->round_amount( $form->{"paid_$i"} * $form->{exchangerate}, $defaultprecision ) - $form->round_amount( $form->{"paid_$i"} * $form->{"exchangerate_$i"}, $defaultprecision ) ) * -1,
        $defaultprecision );
...
sub cogs {
 ...
 ORDER BY i.trans_id|;
  ...
  while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
    ...
    # sellprice is the cost of the item - make sure that we use the defaultprecision
    $linetotal = $form->round_amount( $ref->{sellprice} * $qty, $defaultprecision );
...
sub cogs_returns {
 ...
 ORDER BY i.trans_id|;
  ...
  while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
    ...
    $linetotal = $form->round_amount( $form->{"sellprice_$i"} * $qty, $defaultprecision );
...
sub reverse_invoice {
 ...
 ORDER BY i.trans_id DESC|;
        ...
        while ( $pref = $pth->fetchrow_hashref(NAME_lc) ) {
          ...
          $amount = $form->round_amount( $ref->{sellprice} * $qty, $defaultprecision );
 ...
 GROUP BY ac.approved|;
  ...
  while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
    ...
    $amount = $form->round_amount( $amount, $defaultprecision );
  ...
  for (qw(acc_trans dpt_trans invoice invoicetax inventory shipto vr payment reference)) {
...
sub retrieve_invoice {
 ...
 WHERE pt.parts_id = ?|;
    ...
    while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
      ...
      $ref->{sellprice} = $form->round_amount( $ref->{fxsellprice} * $form->{ $form->{currency} }, $maxpricedecimals );

      # price matrix
      &price_matrix( $pmh, $ref, $form->{transdate}, $maxpricedecimals, $form, $myconfig );
...
sub retrieve_item {
 ...
 WHERE pt.parts_id = ?|;
  ...
  while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
    ...
    # retrieve vendor prices and format them
    # bp 2014/11 get maxpricedecimals
    my $dbh = $form->dbconnect($myconfig);
    %res = $form->get_defaults( $dbh, \@{ ['maxpricedecimals'] } );
    my $maxpricedecimals = $res{maxpricedecimals};
    ...
    # get matrix
    &price_matrix( $pmh, $ref, $transdate, $maxpricedecimals, $form, $myconfig );
...
sub price_matrix {
  my ( $pmh, $ref, $transdate, $maxpricedecimals, $form, $myconfig ) = @_;
  ...
  while ( $mref = $pmh->fetchrow_hashref(NAME_lc) ) {
    ...
    # convert price
    $sellprice = $form->round_amount( $mref->{sellprice} * $form->{ $mref->{curr} }, $maxpricedecimals );
  ...
  if ( $i > 1 ) {
    ...
    $ref->{sellprice} = $form->round_amount( $p{0} * ( 1 - $form->{tradediscount} ), $maxpricedecimals );
