...
sub post_transaction {
  ...
  # bp 2014/11 ensure that we have the right precision (precision of the document currency)
  $form->{fxprecision} = $form->get_precision( $myconfig, $form->{currency} );
  ...
  foreach $accno (@taxaccounts) {
    ...
    # bp 2014/12 - removed logic for the 2nd tax posting for fx documents

    $amount = $tax{fxamount}{$accno} * $form->{exchangerate};
    $tax{amount}{$accno} = $form->round_amount($amount, $form->{precision});

    if ($form->{currency} eq $form->{defaultcurrency}) {
      $taxamt = $tax{amount}{$accno};
    } else {
      $taxamt = $tax{fxamount}{$accno};
    }
  ...
  for $i ( 1 .. $form->{rowcount} ) {
    ...
    if ( $amount{fxamount}{$i} ) {
      ...
      # bp 2014/11 one posting for the whole amount, fxamount in new column

      push @{ $form->{acc_trans}{lineitems} },
        { accno          => $accno,
          amount         => $amount{amount}{$i},
          fxamount       => $amount{fxamount}{$i},
  ...
  $fxinvamount = $form->round_amount( $fxinvamount, $form->{fxprecision} );
  $invamount   = $form->round_amount( $invamount,   $form->{precision} );
  $paid        = $form->round_amount( $paid,        $form->{fxprecision} );

  $paid =
    ( $fxinvamount == $paid ) ? $invamount : $form->round_amount( $paid * $form->{exchangerate}, $form->{precision} );
  ...
  if ($form->{defaultcurrency} ne $form->{currency} ) {
    $calcexrate = ( $invamount / $fxinvamount );


    # bp 2014/11 round the calcuated exchange rate to 10 decimals - will be stored in the ap table
...
# bp 2014/12 we store the more precise exchangerate in table ap
 ...
 WHERE id = $form->{id}|;
  ...
  # add voucher
  if ( $form->{batchid} ) {
    $form->{voucher}{transaction}{vouchernumber} = $form->update_defaults( $myconfig, 'vouchernumber', $dbh )
 ...
 VALUES ($form->{batchid}, $form->{id}, $form->{id}, |
  ...
  foreach $ref ( @{ $form->{acc_trans}{lineitems} } ) {
    ...
    # bp 2014/11 fxamount in additional column -don't mark postings as fx_transaction
    $ref->{fx_transaction} = '0';
    if ( $ref->{amount} ) {
      $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount, fxamount, transdate,
 project_id, memo, fx_transaction, cleared, approved, id)
 VALUES ($form->{id}, (SELECT id FROM chart
 WHERE accno = '$ref->{accno}'),
 $ref->{amount} * $ml * $arapml,
 $ref->{fxamount} * $ml * $arapml,
 '$form->{transdate}',
 ...
 $cleared, '$approved', $voucherid, $paymentid)|;
      ...
      # bp 2014/11 we store the amount paid in table payment

        $query = qq|INSERT INTO payment (id, trans_id, exchangerate,
 paymentmethod_id, fxamount )
 VALUES ($paymentid, $form->{id}, $form->{"exchangerate_$i"},
 $paymentmethod_id, $fxinvamount)|;
        $dbh->do($query) || $form->dberror($query);

        $paymentid++;

        # bp 2014/11 one record for the whole amount
#        if ( $form->{currency} ne $form->{defaultcurrency} ) {
          ...
          $amount = $form->round_amount(
                                         (
                                           $form->round_amount( $paid{fxamount}{$i} * $form->{exchangerate},
                                                                $form->{precision} ) - $form->round_amount(
                                                                       $paid{fxamount}{$i} * $form->{"exchangerate_$i"},
                                                                       $form->{precision}
                                                                )
                                         ) * -1,
                                         $form->{precision} );

  # $amount = $form->round_amount(($form->round_amount($paid{fxamount}{$i} * $form->{exchangerate}, $form->{precision});

          if ($amount) {


            my $accno_id =
              ( ( $amount * $ml * $arapml ) > 0 ) ? $defaults{fxgain_accno_id} : $defaults{fxloss_accno_id};
            $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount,
...
sub get_name {
  ...
  if ( $myconfig->{dbdriver} eq 'DB2' ) {
    ...
    $duedate =
      ( $form->{transdate} ) ? "to_date('$form->{transdate}', '$dateformat') + c.terms" : "current_date + c.terms";
  }
  ...
  # bp 2014/11 added: b.description AS business_type, c.business_id,
  # get customer/vendor
 ...
 b.description AS business_type, c.business_id,
 pr.pricegroup
 $shipto
 FROM $form->{vc} c
 ...
 WHERE c.id = $form->{"$form->{vc}_id"}|;
  ...
  $ref->{arap_accno} .= qq|--|;
  $ref->{arap_accno} .=
    ( $ref->{arap_accno_translation} ) ? $ref->{arap_accno_translation} : $ref->{arap_accno_description};
  $ref->{payment_accno} .= qq|--|;
  $ref->{payment_accno} .=
    ( $ref->{payment_accno_translation} ) ? $ref->{payment_accno_translation} : $ref->{payment_accno_description};
  $ref->{discount_accno} .= qq|--|;
  $ref->{discount_accno} .=
    ( $ref->{discount_accno_translation} ) ? $ref->{discount_accno_translation} : $ref->{discount_accno_description};
  $ref->{payment_method} .= qq|--$ref->{paymentmethod_id}|;

  # bp 2014/11 for setting the business type for payments
  $ref->{business_type} .= qq|--$ref->{business_id}|;
